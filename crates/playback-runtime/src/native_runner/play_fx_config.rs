...
pub(super) fn default_play_fx_selected() -> Value {
    json!({ "fxType": "none", "targetKey": "master", "params": {} })
}

pub(super) fn sanitize_play_fx_config(config: &Value) -> Value {
    let fx_type = match play_fx_type(config) {
        "stutter" | "freeze" | "filter_sweep" | "pitch_shift" => play_fx_type(config),
        _ => "none",
    };
    let target_key = match play_fx_target_key(config) {
        ...
        | "instrument_4" | "instrument_5" | "instrument_6" | "instrument_7" | "instrument_8" => {
            play_fx_target_key(config)
        }
        _ => "master",
    };
    let mut params = serde_json::Map::new();
    for key in play_fx_param_keys(fx_type) {
        let value = config
            .get("params")
            .and_then(|params| params.get(*key))
            .and_then(Value::as_i64)
            .unwrap_or_else(|| i64::from(play_fx_param_default(fx_type, key)));
        params.insert(
            (*key).into(),
            json!(sanitize_play_fx_param(fx_type, key, value)),
        );
    }
    json!({ "fxType": fx_type, "targetKey": target_key, "params": params })
}

fn sanitize_play_fx_param(fx_type: &str, key: &str, value: i64) -> i64 {
    match (fx_type, key) {
...
pub(super) fn play_fx_type(config: &Value) -> &str {
    config
        .get("fxType")
...
pub(super) fn play_fx_target_key(config: &Value) -> &str {
    config
        .get("targetKey")
        .and_then(Value::as_str)
        .unwrap_or("master")
...
pub(super) fn play_fx_params_map(config: &Value) -> serde_json::Map<String, Value> {
    config
        .get("params")
...
pub(super) fn play_fx_params(config: &Value) -> BTreeMap<String, Value> {
    play_fx_params_map(config).into_iter().collect()
}

pub(super) fn play_fx_param_keys(fx_type: &str) -> &'static [&'static str] {
    match fx_type {
...
pub(super) fn play_fx_param_default(fx_type: &str, key: &str) -> i32 {
    match (fx_type, key) {
        ("stutter", "rateHz") => 8,
