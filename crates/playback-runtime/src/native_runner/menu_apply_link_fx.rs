...
use super::{
    ...
    set_target_slot_from_menu, set_u8_enum_from_menu, set_u8_from_menu, NativeLinkLayer,
    NativeRunner, Value,
};
impl NativeRunner {
    #[cfg(test)]
    ...
    pub(super) fn apply_play_fx_menu_state(&mut self) -> bool {
        ...
        let fx_type = self
            .menu
            .value_for_key("play.fx.type")
            .unwrap_or_else(|| play_fx_type(&before).into());
        let target = self
            .menu
            .value_for_key("play.fx.target")
            .unwrap_or_else(|| play_fx_target_key(&before).into());
        let mut params = serde_json::Map::new();
        for key in play_fx_param_keys(&fx_type) {
            let default = play_fx_param_default(&fx_type, key);
            let value = self
                .menu
                .number_for_key(&format!("play.fx.params.{key}"))
                .unwrap_or_else(|| {
                    before
                        .get("params")
        ...
        self.play_fx_selected = json!({ "fxType": fx_type, "targetKey": target, "params": params });
        self.play_fx_selected != before
    }
}

pub(super) fn apply_link_arp_menu_state(
    menu: &crate::native_menu::NativeMenuModel,
    layer: &mut NativeLinkLayer,
    prefix: &str,
) -> bool {
...
pub(super) fn apply_link_scan_and_mapping_menu_state(
    menu: &crate::native_menu::NativeMenuModel,
    layer: &mut super::NativeLinkLayer,
    prefix: &str,
) -> bool {
    let mut changed = false;
...
pub(super) fn apply_link_probability_and_pitch_menu_state(
    menu: &crate::native_menu::NativeMenuModel,
    layer: &mut super::NativeLinkLayer,
    prefix: &str,
) -> bool {
...
pub(super) fn apply_link_axis_menu_state(
    menu: &crate::native_menu::NativeMenuModel,
    layer: &mut super::NativeLinkLayer,
    prefix: &str,
    axis: &str,
