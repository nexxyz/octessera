...
impl EngineSource {
    ...
    pub(super) fn set_source_owned_drop_probe(
        &mut self,
        drop_tx: std::sync::mpsc::Sender<std::thread::ThreadId>,
    ) {
        self.source_owned_drop_probe = Some(SourceOwnedDropProbe { drop_tx });
    }
...
pub(super) fn assert_no_callback_memory_activity(source: &mut EngineSource) {
...
pub(super) fn sample_bank(value: f32) -> SampleBankConfig {
...
pub(super) fn full_sample_source() -> (
    EngineEventSender,
    EngineSource,
...
pub(super) fn drop_retired_state_off_callback(state: RetiredAudioState) -> (usize, usize) {
...
fn full_sample_all_notes_off_has_no_callback_memory_activity() {
    ...
    assert_eq!(drop_retired_state_off_callback(retired), (0, 0));
    assert_eq!(source.engine.profile_snapshot().active_sample_voices, 0);
    let (teardown_allocations, teardown_deallocations) =
        allocations_and_deallocations(|| drop(source));
    assert_eq!((teardown_allocations, teardown_deallocations), (0, 0));
}

#[test]
fn emergency_retirement_slot_preserves_state_when_storage_is_full() {
    let (tx, mut source, _retired_rx) = full_sample_source();
    ...
    tx.send(EngineEvent::AllNotesOff).unwrap();
    assert_no_callback_memory_activity(&mut source);
    assert!(source.emergency_retirement.is_some());
    let source_thread = std::thread::current().id();
    drop(source);
    let drop_thread = drop_rx
        .recv_timeout(std::time::Duration::from_secs(1))
        .expect("emergency retirement drop notification");
    assert_ne!(drop_thread, source_thread);
}

#[test]
fn blocked_emergency_coalesces_panic_and_suppresses_note_on() {
    let (tx, mut source, retired_rx) = full_sample_source();
    for _ in 0..RETIREMENT_QUEUE_CAPACITY {
        source
            .retired_tx
    ...
    assert_eq!(source.engine.profile_snapshot().active_sample_voices, 0);
    assert!(source.emergency_retirement.is_some());
    assert!(source.control_rx.try_recv().is_err());
}

static PANIC_STORM_REPUBLISHES: AtomicUsize = AtomicUsize::new(0);

fn republish_panic_after_consume(emergency_sequence: &AtomicU64) {
    let count = PANIC_STORM_REPUBLISHES.fetch_add(1, Ordering::AcqRel) + 1;
    if count <= realtime_engine::synth::MAX_CONTROL_EVENTS_PER_CALLBACK + 1 {
        emergency_sequence.store(count as u64, Ordering::Release);
    }
}

#[test]
fn panic_storm_stays_within_callback_budget_and_carries_forward() {
    let (tx, mut source, retired_rx) = full_sample_source();
    for _ in 0..RETIREMENT_QUEUE_CAPACITY {
        source
            .retired_tx
    ...
    drop(retired_rx);
    PANIC_STORM_REPUBLISHES.store(0, Ordering::Release);
    source
        .control_rx
        .set_panic_consume_hook(republish_panic_after_consume);
    tx.send(EngineEvent::AllNotesOff).unwrap();
    tx.send(EngineEvent::NoteOn {
        instrument_slot: 0,
        note: 36,
    ...
    let first = source.drain_control_events();
    assert!(first.control_events <= realtime_engine::synth::MAX_CONTROL_EVENTS_PER_CALLBACK as u64);
    assert_eq!(source.engine.profile_snapshot().active_sample_voices, 0);
    assert!(source.emergency_retirement.is_some());
    assert_eq!(
        PANIC_STORM_REPUBLISHES.load(Ordering::Acquire),
        realtime_engine::synth::MAX_CONTROL_EVENTS_PER_CALLBACK
    );

    let second = source.drain_control_events();
    assert!(second.control_events > 0);
    assert!(
        second.control_events <= realtime_engine::synth::MAX_CONTROL_EVENTS_PER_CALLBACK as u64
    );
    assert!(
        PANIC_STORM_REPUBLISHES.load(Ordering::Acquire)
            > realtime_engine::synth::MAX_CONTROL_EVENTS_PER_CALLBACK
