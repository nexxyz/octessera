...
3. **Hardware-qualify the exact internal DAC on both boards.** **Action:** Run the fixed-DAC output check on Raspberry and Orange and record the exact device identity and result. **Evidence:** `docs/open-work.md`; `docs/board-profiles.md`. **Value:** Establishes the real audio path before dependent claims. **Risk:** Hardware-dependent; cabling, image, and ALSA state can block the run. **Sequence:** Before sample or FX qualification.
