...
- `System > Sound` shows `DSP Mode` only when the Orange capacity capability is available, replacing `Output Buffer`. Its values are `Inline / low latency` (default) and `Multicore / capacity`; inline is the lowest-latency path, while multicore adds buffering for capacity. Jack Audio stays on in both modes, and neither mode changes voice or FX limits. The choice persists as `runtimeConfig.sound.optimizeFor`, is retained alongside output-buffer preferences when loading patches or empty state, shows `Restart device to apply`, and uses the existing leave-row `Confirm Reboot` flow; audio is never switched live. Unsupported platforms retain `Output Buffer` with choices `64/128/256/512/1024/2048` and default `256`. Output buffer frames and the internal render quantum remain separate capabilities.
