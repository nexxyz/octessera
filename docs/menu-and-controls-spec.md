...
- Restart-sensitive edits to Audio, USB MIDI, USB Role, and their output settings use the native `Save Setting` / `Restart required.` flow. Its default is `Cancel`, with `Save this setting` when the baseline-plus-setting payload validates and `Save everything` otherwise also available. A Host pre-save warning shows `Restart required.`, `Before Host reboot:`, `Unplug computer USB.`, and `Audio/MIDI/SD2 off.` Auto Save on writes the full current payload once; Auto Save off waits for the explicit choice. A successful Host save shows `Restart?` / `Saved. Reboot to apply?` followed by `Unplug computer USB`, `before Host reboot.`, and `Audio/MIDI/SD2 off.`; non-Host acknowledgements retain the existing two-line dialog. Both use default `Continue` and optional `Reboot now`; stale or failed acknowledgements never reboot or advance the persisted baseline. Setting-only saves preserve unrelated dirty edits. `Start Transfer` and `Stop Transfer` under `System > SD Card 2` retain their existing modal/cancel behavior. General MIDI enable/input/clock/sync remains under `System > MIDI`.
- `System > Audio` exposes USB Audio and HDMI Audio only on Pi boards as optional mirrors of the always-on Jack mix; USB Audio is hidden and disabled in USB Host mode, while HDMI Audio remains visible. `System > MIDI` contains the global Enabled gate, board-only USB MIDI, MIDI Out, MIDI In, and Sync / Clock. Enabled is the runtime MIDI gate and selects no port or device; USB MIDI is hidden and disabled in USB Host mode, otherwise it enables the bidirectional class device after restart, auto-routes outbound gadget MIDI, and leaves inbound selection under MIDI In. `System > USB Role` is Raspberry-capability-only; Host also hides SD Card 2 Start Transfer while retaining Stop Transfer for cleanup, and Gadget does not restore previously disabled outputs. `System > SD Card 2` contains Start Transfer and Stop Transfer; the second card is exposed to a USB host only while conflicting USB audio, MIDI, and recording are inactive. `System > HDMI Video` contains the existing HDMI display controls; SD Card 2 and HDMI Video are board-only. HDMI Audio is independent of HDMI video. Simultaneous physical outputs use independent unsynchronized clocks and can drift or echo; this phase does not provide sample alignment.
