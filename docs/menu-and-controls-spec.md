...
- Persistent-worker CPU icon: red at `(117,5)` only when valid persistent worker evidence reports `high_cpu_steady`; missing or inline evidence hides it. The save icon remains yellow at `(107,5)` and may coexist. Missed-quantum flash remains false until Phase 2.
