...
| Combined modifier + leftmost grid column | Layer trigger gate toggle | Toggles that layer between `0%` and its previous trigger mode without changing active layer. Only this disable transition immediately releases already-emitted notes owned by that layer, including internal synth/sample and external MIDI notes; ordinary Play Trigger Gate edits to `0%` only suppress future admissions. Other layers are unaffected, and re-enable does not resurrect released notes.
...
| Shift + Fn + leftmost grid column | Shift+Ctrl + leftmost grid column | Toggle that layer's trigger gate without changing active layer; disabling releases its owned internal synth/sample and external MIDI notes and suppresses future triggers |
