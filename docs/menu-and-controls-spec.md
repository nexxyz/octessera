...
- Restart-sensitive edits to Audio, USB, and their output settings use the native `Save Setting` / `Restart required.` flow. Its default is `Cancel`, with `Save this setting` when the baseline-plus-leaf payload validates and `Save everything` otherwise also available. Auto Save on writes the full current payload once; Auto Save off waits for the explicit choice. A successful save shows `Restart?` / `Saved. Reboot to apply?` with default `Continue` and optional `Reboot now`; stale or failed acknowledgements never reboot or advance the persisted baseline. Setting-only saves preserve unrelated dirty edits. `Start SD2 Xfer` and `Stop SD2 Xfer` retain their existing modal/cancel behavior. General MIDI enable/input/clock/sync remains under `System > MIDI`.
- `System > Audio` exposes USB Audio and HDMI Audio only on Pi boards as optional mirrors of the always-on Jack mix; Jack, USB, HDMI, and SD2 device controls are hidden on desktop. `System > USB` contains USB MIDI and SD2 transfer controls and `System > HDMI Video` contains the existing HDMI display controls; both groups are board-only. HDMI Audio is independent of HDMI video. Simultaneous physical outputs use independent unsynchronized clocks and can drift or echo; this phase does not provide sample alignment.
