...
- `System > Configure WiFi` uses stable key `system.configureWifi` and opens only after confirmation. Confirmation stops and resets playback, clears link/arp note state, and sends MIDI panic/all-notes-off cleanup; playback never resumes automatically. The native runtime then emits the typed `RuntimePlatformEffect::SetupPortalOpen` effect and presents typed `RuntimeSetupPortalStatus` phases: `starting` (request accepted and hotspot starting), `portal_ready` (shows the four-character suffix and `192.168.42.1` for 30 minutes), `finalizing` (settings are applying), `succeeded` (complete; no reboot), `failed` (settings may be partial), `timed_out` (the 30-minute portal closes), and desktop `unsupported`. On Pi, the setup adapter also starts a short-lived local Data Backup/Restore transfer service and returns its URL and transfer code in the typed status; the native OLED modal shows both. Export includes named patches, current/default state, and the allowlisted preference delta, with optional custom samples and persisted recordings. Restore validates and stages the complete archive before mutation and requires physical confirmation; that confirmation is consumed atomically, emits `RuntimeUserDataRestoreStatus { phase: RuntimeUserDataRestorePhase::Restoring }`, shows `Restoring...` and `Please wait`, rejects normal device input until the worker reports `Succeeded` or `Failed`, and leaves ambiguous recovery trees and the pre-restore archive intact. The transfer service is Pi-only; desktop remains unsupported. The portal can change Wi-Fi, hostname, SSH access, and the board's admin login (`pi` on Raspberry; `octessera` on Orange); there is no automatic reboot. Hide suppresses only the current OLED phase and does not cancel the request; the modal reappears when the phase changes, including the terminal result. A retry requires a new `Configure WiFi` action.
...
- `System > Recording` persists settings only: `runtimeConfig.recording.maxMinutes` defaults to `10` and clamps to `1..120`. `Start Audio` emits `RuntimePlatformEffect::RecordingStartAudio { max_minutes }`; `Stop` emits `RuntimePlatformEffect::RecordingStop`. On Pi, recording captures the final internal stereo output as 44.1 kHz/16-bit WAV files under `/home/pi/recordings/`. It does not capture external MIDI instrument audio, OLED frames, display SD storage, USB audio input, or MIDI input.
