...
- `System > Sound` shows `DSP Mode` only when the Orange capacity capability is available, replacing `Output Buffer`. Its values are `Inline / low latency` (default) and `Multicore / capacity`; inline is the lowest-latency path, while multicore adds buffering for capacity. Jack Audio stays on in both modes, and neither mode changes voice or FX limits. The choice persists as `runtimeConfig.sound.optimizeFor` and is retained alongside output-buffer preferences when loading patches or empty state. Output-buffer changes show `Restart device to apply` and open `Confirm Reboot` when leaving the row; a changed DSP Mode value opens the existing `Confirm Audio` dialog with `Save / Reboot` immediately. Audio is never switched live. Unsupported platforms retain `Output Buffer` with choices `64/128/256/512/1024/2048` and default `256`. Output buffer frames and the internal render quantum remain separate capabilities.
- `System > Audio / USB` persists the exact schema-v2 `runtimeConfig.audioOutputs` set `{dac, usb, hdmi}` and `runtimeConfig.usb.midiOutEnabled` (default Jack Audio on, USB Audio and HDMI Audio off; USB MIDI off). On both Pi profiles, every valid set includes always-on Jack; USB Audio and HDMI Audio remain visible as independently selectable optional mirrors of the canonical Jack mix, while the no-op Jack row is omitted. Readiness depends on Jack. An absent, waiting, or faulted optional mirror is route-local and never blocks or replaces Jack. On desktop, the existing Jack control and flexible output representation remain available. HDMI Audio is independent of HDMI video. Simultaneous physical outputs use independent unsynchronized clocks and can drift or echo; this phase does not provide sample alignment. Changes show `Audio: Save / Reboot`. That action opens the native confirmation modal; Cancel dismisses it without saving or rebooting, while Save / Reboot emits one canonical full device payload, saves it before requesting reboot, and does not reboot when the save fails. `Start SD2 Xfer` and `Stop SD2 Xfer` retain their existing modal/cancel behavior. General MIDI enable/input/clock/sync remains under `System > MIDI`.
