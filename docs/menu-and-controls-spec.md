...
- `System > Sound > Output Buffer` persists Pi CPAL/ALSA output buffer frames as `runtimeConfig.sound.audioOutputBufferFrames` with choices `64/128/256/512/1024/2048`, menu default `256`. Changing it shows `Restart device to apply`; leaving the edited row opens the standard `Confirm Reboot` dialog. Audio is not reopened live. On Orange, direct CPAL uses the same 256-frame fallback when no persisted value is present; explicit persisted choices remain in force. Output buffer frames and the internal render quantum are separate capabilities: Orange no longer derives the render quantum from output size, and uses the provisional 128-frame `audioRenderQuantumFrames` default unless `OCTESSERA_AUDIO_RENDER_QUANTUM_FRAMES` is explicitly set. The Phase 1 capability default is a 128-frame internal render quantum and is not a menu/runtime setting. On Pi startup, `OCTESSERA_AUDIO_OUTPUT_BUFFER_FRAMES` and `OCTESSERA_AUDIO_RENDER_QUANTUM_FRAMES` remain higher-priority development/profiling overrides for their respective settings.
