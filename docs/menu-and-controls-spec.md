...
- `System > Reboot` and `System > Shutdown` confirm before emitting their native effects, after the recovery-save effect when recovery state exists. Orange performs external MIDI panic and internal audio silence before either power action; a safety failure prevents the action and remains a typed ordinary failure. Reboot and Shutdown share the fixed root-owned `/run/octessera-device-apply/reboot.sock` protocol with exact `reboot\n` and `poweroff\n` requests and exact `accepted\n`/`rejected\n` responses. Orange reboot still validates persisted config in the root helper; poweroff does not depend on that config validation.
