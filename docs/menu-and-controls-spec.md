...
- Steady high-CPU presentation uses persistent worker evidence only: the configured threshold is immediate, clearing requires utilization strictly below `threshold - 5 percentage points` for five rendered seconds, and equality remains in the hold band and resets clear accumulation. Absent worker evidence hides the icon. `missed-quantum` flash presentation is reserved for recoverable overload handling in Phase 2; current terminal `DeadlineMiss` status does not produce a flash.
