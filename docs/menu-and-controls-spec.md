...
|---|---|---|
| Shift + Space | Stop | Internal sync: stop/reset, silence internal audio, and clear held MIDI notes without broad panic.
...
| Combined modifier + leftmost grid column | Layer trigger gate toggle | Toggles that layer between `0%` and its previous trigger mode without changing active layer. Only this disable transition releases held Synth, FM, Plucked, and Sampler notes owned by that layer, plus external MIDI notes; an already-ringing one-shot Drum hit finishes its decay. Ordinary Play Trigger Gate edits to `0%` only suppress future admissions. Other layers are unaffected, and re-enable does not resurrect released notes.
...
| Shift + Space | Shift+Space | Stop/reset with internal-audio silence and bounded held-note cleanup; external sync arms the next 96-PPQN boundary instead |
...
System children are ordered as Load Preset, Recording, Notes, MIDI, Audio, UI, conditional SD Card 2,
conditional HDMI Video, Saves, Setup, Reset, then Panic, Sys. Info, Basic Help, Reboot, and Shutdown.
Within each recursive group, parameters and submenus precede action rows, except that the dynamic Rename group lists preset choices before its Rename actions. The System tree fully expands
the desktop, Orange, Raspberry Gadget, and Raspberry Host branches.
...
- Context help for every submenu, parameter, action, and non-file selector menu item must resolve to an entry from `resources/menu-help-texts.tsv`; native tests must fail on missing coverage. Behavior selector leaves use behavior-specific help, behavior category menu items use stable `behavior.category.*` help keys, and parameter binding picker leaves resolve to the underlying target parameter help. Dynamic file menu items such as sample browser files and folders keep their own `sample.*` action help.
...
- Aggregate audio-load telemetry is separate from worker warning presentation. Recent voice stealing turns the active event dot magenta; aggregate load and voice-steal status remain on the desktop audio-load bridge and do not drive the worker icon.
- Persistent-worker CPU icon: red at `(117,5)` only when valid persistent worker metrics report `high_cpu_steady`; missing or inline metrics hide it. A missed-quantum flash has priority in that same CPU slot and uses the inverted white/black CPU glyph, so the red CPU icon is hidden while the miss is active. The save icon remains yellow at `(107,5)` and may coexist with either CPU-slot state. A newly missed persistent quantum replays the previous final master quantum exactly once when its frame count matches; otherwise it emits silence, and subsequent refills during recovery emit silence. `missedQuantumFlash` is armed for five emitted seconds (`sampleRate * 5`), clears on the exact emitted-frame crossing, and resets on a later miss. These status transitions do not change the existing OLED layout or coordinates.
...
- Number/enum/bool rows enter edit mode on Main press; Main or Back finishes the row. At the Save Setting choice, Back means Cancel; at the Restart? choice, Back means Continue.
- Navigation memory is limited to `System`, `System > Notes`, `System > Audio`, `System > Audio > Engine`, and `System > UI`. It is native, ephemeral, cleared on menu rebuild, and does not apply to any other menu, dynamic list, sample browser, preset list, MIDI port list, parameter picker, help, confirm dialog, or assignment overlay.
- `System > Audio > Engine` persists `runtimeConfig.dsp.workerWarningThreshold` as `70/75/80/85/90/95` with default `85`, and `runtimeConfig.dsp.busIdleThreshold` as `exact/-140/-120/-100/-80` with default `-120`. These typed settings propagate directly to the realtime engine without rebuilding prepared audio or changing structural render-plan revisions. Worker warning presentation and bus silence behavior are separate runtime phases.
- Steady high-CPU presentation uses persistent worker metrics only: the configured threshold is immediate, clearing requires utilization strictly below `threshold - 5 percentage points` for five rendered seconds, and equality remains in the hold band and resets clear accumulation. Absent worker metrics hide the icon. `DeadlineMiss` is native, nonterminal ownership recovery; a valid previous final master quantum is repeated once on the first matching missed quantum, then pending refills are silent until the late state recovers. `missedQuantumFlash` remains a native status flag for five emitted seconds, including exact boundary clearing and reset on a later miss; it does not alter the existing visual structure. Structural worker failures (`DispatchFailed`, `CompletionFailed`, `WorkerExited`, and `InvalidBlock`) remain sticky terminal states and do not enter this recovery path.
- `System > Notes` keeps the existing `sound.noteLengthMs`, `sound.velocityScalePct`, and `sound.velocityCurve` keys and semantics. `System > Audio > Perf. Mode` persists `runtimeConfig.sound.optimizeFor` as `latency|capacity`; its compact OLED values are `Lat` and `Cap` for the full Latency and Capacity choices, it is visible on Raspberry and Orange, and it is hidden on desktop. `System > Audio > Polyphony` keeps the existing `sound.voiceStealingMode` values `fixed12/fixed16/auto-soft/auto-balanced/auto-hard/none`; `Engine` keeps the existing DSP threshold keys and values. Neither performance mode changes voice or FX limits.
- Restart-sensitive edits to Audio, `System > MIDI > USB Device > USB MIDI`, `System > Setup > USB Role`, and their output settings use the native `Save Setting` / `Restart required.` flow. Its default is `Cancel`, with `Save this setting` when the baseline-plus-setting payload validates and `Save everything` otherwise also available. A Host pre-save warning shows `Restart required.`, `Before Host reboot:`, `Unplug computer USB.`, and `Audio/MIDI/SD2 off.` Auto Save on writes the full current payload once; Auto Save off waits for the explicit choice. A successful Host save shows `Restart?` / `Saved. Reboot to apply?` followed by `Unplug computer USB`, `before Host reboot.`, and `Audio/MIDI/SD2 off.`; non-Host acknowledgements retain the existing two-line dialog. Both use default `Continue` and optional `Reboot now`; stale or failed acknowledgements never reboot or advance the persisted baseline. Setting-only saves preserve unrelated dirty edits. `Start Transfer` and `Stop Transfer` under `System > SD Card 2` retain their existing modal/cancel behavior. General MIDI enable/input/clock/sync remains under `System > MIDI`.
- `System > Audio` exposes conditional USB Audio, conditional HDMI Audio, Master Vol, conditional Perf. Mode, Polyphony, and Engine in that order; the output rows are optional mirrors of the always-on Jack mix and are only on Pi boards. USB Audio is hidden and disabled in USB Host mode, while HDMI Audio remains visible. `System > MIDI` contains `MIDI Active`, then conditional `MIDI Host` and `USB Device` groups, then Sync / Clock. MIDI Active is the global runtime gate and selects no port or device. MIDI Host contains MIDI Out then MIDI In and is shown on desktop and Orange, and on Raspberry only in Host mode. USB Device contains USB MIDI and is shown on Orange and on Raspberry only in Gadget mode; it is the computer-facing gadget interface and automatically owns both Gadget MIDI input and output while enabled. Host-selected input/output IDs are ignored while USB Device MIDI is enabled; on Orange they are retained for later use after it is disabled. USB Device changes are restart-sensitive: pending edits do not change the live route until the next process startup. `System > Setup > USB Role` is Raspberry-capability-only and precedes Updates. Host also hides SD Card 2 Start Transfer while retaining Stop Transfer for cleanup, and Gadget does not restore previously disabled outputs. `System > SD Card 2` contains Start Transfer and Stop Transfer; the second card is exposed to a USB host only while conflicting USB audio, MIDI, and recording are inactive. `System > HDMI Video` contains the existing HDMI display controls; SD Card 2 and HDMI Video are board-only. HDMI Audio is independent of HDMI video. Simultaneous physical outputs use independent unsynchronized clocks and can drift or echo; this phase does not provide sample alignment.
- `System > Reboot` and `System > Shutdown` confirm before emitting their native effects, after the recovery-save effect. One board-neutral terminal lifecycle requires that save to succeed, atomically stops non-terminal follow-ups and new input, independently attempts external MIDI panic and internal audio silence, then only after both succeed publishes and physically acknowledges the final shutdown/reboot snapshot and LED/OLED state before submitting the board-specific fixed power request. Save, safety, terminal acknowledgement, and power-submission failures are typed/logged and never resume normal follow-ups incorrectly. The exact ordinary-menu toasts remain `Rebooting` and `Shutting down`; the OLED frame stays preserved while native hardware teardown completes. Raspberry uses the exact noninteractive power commands allowed by `/etc/sudoers.d/octessera-shutdown`, with no broad `NOPASSWD` grant. Orange uses its fixed root-owned `/run/octessera-device-apply/reboot.sock` with exact `reboot\n` and `poweroff\n` requests and exact `accepted\n`/`rejected\n` responses. Orange reboot still validates persisted config in the root helper; poweroff does not depend on that config validation. This bounded contract applies to confirmed instrument-menu actions, not arbitrary administrative `systemctl poweroff` or `systemctl reboot` commands.
- `System > Setup > Updates > Check` emits `RuntimePlatformEffect::UpdateCheck` without confirmation. On supported Raspberry Pi hosts, the updater reports whether the GitHub release contains the installed profile's ZIP/checksum pair. On Orange, Check/Apply/Rollback use the root-owned broker and guarded updater with the explicit profile-qualified `octessera-<version>-orange-pi-zero-2w-runtime-updater-aarch64.zip` and `SHA256SUMS-orange-pi-zero-2w-runtime-updater.txt` pair. These actions update only the managed runtime release; full Armbian, kernel, device-tree, and image replacement remains manual, and the standalone manual runtime ZIP is not an OTA asset. Orange profile, asset, manifest, checksum, and health mismatches fail closed; it never consumes Raspberry assets or falls back to the manual ZIP or image path. On desktop, Check opens the GitHub releases page; desktop Apply and Rollback report unsupported. On supported Raspberry and Orange hosts, Apply and Rollback confirm first, stage the candidate or previous runtime release, and schedule guarded validation. The immediate Apply result may be `Update health validation scheduled.`; it is not a completed-update result. The guard validates process identity and stability before committing; downloaded Apply candidates also require profile/version readiness, and a failed validation automatically restores the previous release. Legacy installations must be provisioned or updated with the current OS bundle, or reflashed, before online apply; the legacy updater must not apply new releases.
- On the Orange production runtime, native musical events, runtime audio commands, and silence route through the realtime engine at the shared 44.1 kHz runtime rate. Jack is always-on primary; selected USB or HDMI audio routes are optional mirrors of the canonical Jack mix. Readiness depends on Jack; an absent, waiting, or faulted optional mirror is route-local and never blocks or replaces Jack. HDMI audio is separate from HDMI video, and simultaneous physical outputs are not sample-synchronized. MIDI uses the native host adapter, including USB MIDI when the configured gadget port is present. The service polls NeoTrellis/NeoKey over `/dev/i2c-2` at about 10 ms and uses the native gpiocdev v2 encoder path for all four encoders. Orange HAL descriptors reverse each literal board A/B net at the event boundary so all four produce canonical turn direction without changing shared quadrature decoding. AUX2 A/B offsets 227/269 are always requested; switch offset 224 is omitted only while UART0 is active and is requested once the dedicated UART0-disabled input-routing overlay is applied. Lower-left grid and GRB output semantics remain unchanged.
- `System > Sys. Info` opens a native loading popup and emits `RuntimePlatformEffect::SystemInfoRequest`. Desktop and Pi return an asynchronously identified, sanitized `RuntimeSystemInfo` containing OS/version, Octessera version, primary IP/MAC when available, hostname, and explicit board profile. The desktop UI only renders the resulting snapshot.
- `System > Setup > Configure WiFi` uses stable key `system.configureWifi` and keeps its existing confirmation and native setup-portal behavior. `System > Setup > Backup / Restore` uses stable key `system.backupRestore` and remains a direct, unconfirmed action. Updates and Hardware Test remain under Setup without action-key changes.
- `System > HDMI Video` displays `Mode` as `Terminal | live-grid | plain-grid | active-behavior | cycle-behaviors` and persists the canonical `runtimeConfig.hdmi.mode` value (`none|live-grid|plain-grid|active-behavior|cycle-behaviors`, default `none`). `Terminal` is the compatibility label for stored/runtime `none`; it releases and disables Octessera framebuffer output so Linux terminal ownership can show, while snapshots retain their existing black/inactive HDMI grid semantics. `Bars per cycle` appears only in `cycle-behaviors` mode.
...
- `System > Recording` persists `runtimeConfig.recording.maxMinutes` with default `10` and clamps to `1..120`; the same limit applies to WAV and Audio+OLED AVI. `Start Audio` emits `RuntimePlatformEffect::RecordingStartAudio { max_minutes }` and writes 44.1 kHz stereo signed-16 little-endian PCM WAV. `St. Audio+OLED` emits `RuntimePlatformEffect::RecordingStartAudioOled { max_minutes }` and writes an AVI with 128x128 MJPG video at 10 fps plus 44.1 kHz stereo signed-16 little-endian PCM audio. Audio is the master clock, so each video frame advances every 4,410 audio frames and accepted OLED frames are held between updates. `Stop` emits `RuntimePlatformEffect::RecordingStop` and is safe when idle. Both actions capture the final internal stereo output; external MIDI instrument audio is not captured.
...
- Recording feedback is native: a successful start shows `Recording started`; a duplicate start shows `Recording is already running`; manual Stop shows `Recording saved` or `Recording incomplete`; clean max-duration completion shows `Max time: saved`. While active, recording keeps the OLED awake without changing the saved `OLED Sleep` setting, and normal idle sleep timing restarts when recording ends.
...
- `Shape > Instruments > I# > Type` offers `fm` (shown as `FM`) alongside synth, sampler, and MIDI. An FM slot keeps Type, Note Mode, FM, Mixer, Auto Label, Name, and Slot Actions in that order. FM opens to six rows: `Tone >`, `Index Env >`, `Filter >`, `Volume >`, `Amp Env >`, `Filter Env >`; all fit below the OLED title. Tone holds Ratio (`0.5|1|2|3|4|5|6|8`, default `2`, displayed as `1:2|1:1|2:1|3:1|4:1|5:1|6:1|8:1`) and Index (`0..100`, default `50`, mapped to `0..4` radians). Index Env starts at Attack 0 ms, Decay 250 ms, Sustain 20%, Release 120 ms. Volume starts at Gain 80%, Vel Sens 100%; Amp Env starts at 5/300/70/350. FM Filter and Filter Env use Synth's control scales and defaults, saved under this slot's own `fm` block. Use the familiar edit encoder and Back to nudge the tone; FM audio follows the same Mixer and FX bus path as Synth, while MIDI remains external.
- FM parameter binding pickers expose Index, Volume Gain, Filter Cutoff, and Filter Res as numeric targets for modulation, XY, and Aux. Their saved labels carry an `FM` prefix and fit on the OLED's selected Current row. Ratio and Filter Type stay menu selectors, not continuous binding targets.
- `Shape > Instruments > I# > Type` also offers `pluck` (shown as `Plucked`). A Plucked slot keeps Type, Note Mode, Plucked, Mixer, Auto Label, Name, and Slot Actions in that order. Plucked opens to exactly five rows: `String >`, `Filter >`, `Volume >`, `Amp Env >`, `Filter Env >`, all below the OLED title. String has Decay (`100..5000 ms`, default `1500`), Brightness (`0..100%`, default `65`), and Pick Pos (`5..50%`, default `25`). Volume starts at Gain 80% and Vel Sens 100%; Amp Env starts at 0/0/100/900. Filter and Filter Env keep Synth's scales and defaults but save under this slot's `pluck` block. Use the normal edit encoder and Back. Plucked audio uses the same Mixer and FX bus path as Synth and FM.
- Plucked binding pickers offer numeric Decay, Brightness, Pick Pos, Gain, Filter Cutoff, and Filter Res. Decay and Brightness adjust ringing strings; Pick Pos shapes the next pluck and does not change a note already ringing. Filter Type and instrument Type are not smooth numeric targets.
- Sampler binding pickers offer Tune, Gain, Base Velocity, velocity sensitivity/levels, and Filter Cutoff/Res as audible audio targets. Filter Type, Env Amount, Key Tracking, Amp Env, and Filter Env remain editable and saved in the Sampler menu, but are not currently rendered as sampler audio controls and are not offered as new binding targets.
- `Shape > Instruments > I# > Type` also offers `drum` (shown as `Drum`). A Drum slot shows Type, Drum, Mixer, Auto Label, Name, Slot Actions; Note Mode is hidden because hits are always one-shot. Drum has exactly seven rows: Voice, `Edit >`, Assign, Cell Tune, Preview, `Filter >`, `Volume >`. Voice lists `V1: Kick` through `V8: Rim`, deriving each name from its current Sound. Edit has Sound (`kick`, `snare`, `closed_hat`, `open_hat`, `low_tom`, `high_tom`, `clap`, `rim`), Tune (-12..12 semitones), Decay (20..2000 ms), Tone (0..100%), and Attack (0..50 ms). Sound resets only that voice's Tune/Decay/Tone/Attack to its style defaults; other voices and cell offsets stay put. Drum keeps its own Filter and Volume values using Synth's control scales, with default Gain 80% and Vel Sens 100%; no Amp Env or Filter Env page or Kit Load appears in the initial Drum UI.
- Assign maps the selected voice onto grid cells with the Sample Assignment gestures: plain press sets/replaces or toggles that cell, Shift+press edits its whole world-space row (bottom row `y=0`), combined modifier+press edits its column. Assignments start empty, never auto-fill the grid; Back exits without undoing work. Cell Tune picks an assigned cell and edits that cell's -24..24 semitone offset; empty cells report `No drum here`, and Back returns to selection, then exits. Preview plays the selected voice at its own Tune with zero cell offset through the main encoder, without Space or the sample browser. Numeric kit Tune/Decay/Tone/Attack bindings change **next hits**, never ringing hits; slot Gain/Filter Cutoff/Res remain live targets. Voice, Sound, cell offsets, Filter Type, and instrument Type are not smooth numeric targets.
- `Play > Drums` is seventh in the Play page list (Fn+right world `y=6`), leaving rows 0..5 alone. `Slot` displays `I#: Drum` for existing Drum slots and initially selects the first one for this session. The seventh Play row reads `No Drum slot` when none exists; the menu does not add a selectable guidance row. Assigned grid presses on the Drums page play one immediate hit even when stopped; releases are consumed. Empty cells/non-Drum slots are silent. This direct performance mode does not also invoke behavior, Link, or trigger probability. Outside it, behavior-triggered Drum hits still follow their normal Link/probability/timing path.
- Parameter target pickers mirror the main menu root order (`Build`, `Link`, `Shape`, `Play`, `System`) so modulation, Aux, and XY target browsing use the same mental model as normal navigation. Within `Build`, Behavior targets are generated per layer: layers with behavior `none` expose no behavior targets; real behavior layers expose their own Step Rate as `layers.N.algorithmStep` and config fields/actions as `layers.N.build.behaviorConfig.*`.
...
- `Play > XY` is ordered `X Axis`, `Y Axis`, `Smoothing`, `Invert X`, `Invert Y`, `Release`. `Smoothing` is `Off` at `0`, or `10–500 ms` in `10 ms` steps with a shipped default of `80 ms`; it displays `Off` or `<n> ms`. Raw XY markers update immediately, while mapped normalized axes glide independently from their current mapped values without overshoot before target range/step conversion. Elapsed time is monotonic; sample-hold retains and completes an active glide, reset-center moves the marker immediately to center and glides mapped axes to `0.5` when enabled, and inversion retargets only the affected mapped axis while the marker remains non-inverted.
...
- Link collapses only same-tick NoteOn duplicates within one CellTriggerKind group and mapping context, after finalized instrument routing and before Delay/Retrig scheduling; it keeps the first event and duration and uses the highest velocity. Other event kinds, lanes, ticks, layers, mappings, and durations remain distinct.
...
- Bar display applies automatically to FX params, synth/FM/sample shaping controls, editable mixer volume/pan, FX bus volume/pan, Play FX controls, system sound/UI controls, Link axis controls, and behavior controls such as spawn interval/count, threshold, lifespan, and radius
- `Play > FX > Pitch Shift` exposes `Semitones`, `Cents`, `Slide In`, `Slide Out`, and `Mix`. Slide times are `10..3000 ms` in `10 ms` steps with defaults of `120 ms` and `180 ms`; pitch amount glides linearly in octaves, independently of the fixed `PITCH_FILL_FRAMES=1088` live-input fill and fixed 10 ms wet/dry anti-click envelopes.
...
- Structural selector edits apply immediately while the menu item is in edit mode through key-specific fast paths. This covers instrument type, instrument route, FX bus slot type, and master FX slot type. Behavior selection applies immediately when a behavior action item is pressed. Dynamic parameter menu items also apply immediately while editing.
...
- Bus Duck FX exposes `Source`, `Source Tap` (`pre|post`, displayed as `Pre|Post`), `Threshold` 0..1, `Amount %` 0..100, `Attack` 1..500 ms, and `Release` 1..5000 ms. For an Instrument source, `Pre` is the raw source before its fader; `Post` applies its fader before instrument-target momentary FX and pan. For a Bus source, `Pre` is the bus input before its fader and Slot 1→2→3 chain; `Post` applies that bus fader to the same pre-chain input. A bus input already includes routed instruments' faders and instrument-target momentary FX. Both modes exclude source-bus slot FX, bus-target momentary FX, spread/pan, global FX, and master volume. Missing `sourceTap` defaults to `pre` for legacy patches. These are the canonical menu, saved-config, and realtime ranges; accepted values are passed to the ducking renderer without a narrower downstream range.
...
- While any Play Page (`mix`, `pan`, `fx`, `trigger-gate`, `transpose`, `xy`, `drums`) is active: grid shows the Play performance overlay instead of active behavior cells. Play Transpose uses the left column for eligible layer selection, Shift + left column to enable/disable all eligible layers, and columns 1..7 as a three-octave piano offset picker for Synth, FM, Plucked, and enabled MIDI note targets; Sampler and Drum are not transposed. In the transpose picker, the selected offset is green, the unselected center key is white, and available offsets are dim blue. Held transposed notes are safely drained with exact routed note-offs when transpose routing is retargeted, disabled, stopped, or reset.
- Drum assignment and Cell Tune overlays take priority after FX and Sample assignment but before probability assignment and Play Drums. The selected kit voice's assigned cells are bright white, other assigned cells dim white, and empty cells dark; the winning assignment overlay paints after Play and hides Fn navigation LEDs. World coordinates keep the bottom-left origin and the LED display conversion.
- In `Play`, `Mix`, `Pan`, `Trigger Gate`, and `Transpose` act as page-select rows: main encoder press selects and activates the page without entering an empty submenu. `FX`, `XY`, and `Drums` remain enterable when they expose configuration rows.
...
- FM Tone maps Index, Cutoff, Res, Gain to A1–A4; Plucked String maps Decay, Brightness, Pick Pos, Gain. Drum Voice/Edit maps Tune, Decay, Tone, Attack for the selected kit voice. Each type's Filter maps Cutoff, Res, Env, Key; Volume maps Gain, Vel Sens to A1–A2. Sampler Filter maps only audible Cutoff and Res to A1–A2; A3–A4 are empty, and Sampler Amp Env/Filter Env have no automatic mappings because those saved controls are not rendered. Sampler Volume and sample-slot mappings remain available. FM Index, Plucked Decay, and selected Drum voice Tone turns emit typed controls without reinitializing a whole slot.
