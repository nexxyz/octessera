...
- `System > Sound` uses the compact labels `Vel Scale`, `Vel Curve`, `Voices`, `DSP`, and `Buf Frames`. On Orange-capability hosts, `DSP` replaces `Buf Frames` and its values are `Inline` (default) and `Multicore`; on other platforms, `Buf Frames` offers `64/128/256/512/1024/2048` with default `256`. The choice persists as `runtimeConfig.sound.optimizeFor` and is retained alongside output-buffer preferences when loading patches or empty state. Jack Audio stays on in both modes, and neither mode changes voice or FX limits. Output buffer frames and the internal render quantum remain separate capabilities.
- Inline execution is the shipped default on both Pi boards; only Orange exposes the `DSP` selector with its existing `Inline` and `Multicore` values, and `Multicore` selects the Capacity/routing profile and is restart-sensitive. Raspberry remains Inline-only. The existing menu labels and options are unchanged.
- Restart-sensitive edits to Sound, Audio / USB, and USB MIDI use the native `Save Setting` / `Restart required.` flow. Its default is `Cancel`, with `Save this setting` when the baseline-plus-leaf payload validates and `Save everything` otherwise also available. Auto Save on writes the full current payload once; Auto Save off waits for the explicit choice. A successful save shows `Restart?` / `Saved. Reboot to apply?` with default `Continue` and optional `Reboot now`; stale or failed acknowledgements never reboot or advance the persisted baseline. Setting-only saves preserve unrelated dirty edits. Reboot now reuses the ordinary recovery-save, safety, and terminal reboot lifecycle; it never emits `ApplyDeviceConfigReboot`. `Start SD2 Xfer` and `Stop SD2 Xfer` retain their existing modal/cancel behavior. General MIDI enable/input/clock/sync remains under `System > MIDI`.
- `System > Audio / USB` persists the exact schema-v2 `runtimeConfig.audioOutputs` set `{dac, usb, hdmi}` and `runtimeConfig.usb.midiOutEnabled` (default Jack Audio on, USB Audio and HDMI Audio off; USB MIDI off). On both Pi profiles, every valid set includes always-on Jack; USB Audio and HDMI Audio remain visible as independently selectable optional mirrors of the canonical Jack mix, while the no-op Jack row is omitted. Readiness depends on Jack. An absent, waiting, or faulted optional mirror is route-local and never blocks or replaces Jack. On desktop, the existing Jack control and flexible output representation remain available. HDMI Audio is independent of HDMI video. Simultaneous physical outputs use independent unsynchronized clocks and can drift or echo; this phase does not provide sample alignment.
