...
- Orange's `/run/octessera-device-apply/reboot.sock` is a root-owned, group-limited systemd socket for one exact request: `reboot\n`.
- The socket service validates the regular `/var/lib/octessera/presets/default.json` file, its expected runtime ownership/mode, and the canonical audio/gadget schema, then invokes the exact `/usr/bin/systemctl reboot` successfully before returning `accepted\n`; malformed requests and definitive validation or scheduling failures return `rejected\n` without invoking another command.
- Native Rust owns the confirmed save/apply semantic and sends this fixed request through the adapter lane. The image helper owns only validation and reboot execution; it does not accept command arguments, config payloads, or arbitrary commands. Orange has no `octessera-runtime` sudoers entry.
