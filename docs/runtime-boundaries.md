...
- Normal Orange output forwards accepted native RGB565BE frame bytes to the OLED transport without another pixel rotation. The Orange Python boot splash renders through its own canvas-to-controller transform; it is not a native-frame consumer. Raspberry keeps its board-specific HAL rotation.
