...
function Invoke-OrangeCell {
  ...
  if ($Kind -eq "offline") {
    ...
    $arguments += @("-Mode", "LiveAudioBenchmark", "-WorkerTimingMode", "enabled", "-Scenario", $Cell.scenario, "-OutputFrames", [string]$Cell.output_frames, "-EngineBlockFrames", [string]$Cell.internal_frames, "-MeasureSeconds", [string]$Cell.measure_seconds)
  }
