...
$u16Selection = Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_24" -OutputFrames 128 -EngineBlockFrames 32 -MeasureSeconds 30 -ExecutorMode "inline" -WorkerTimingMode "disabled"
if ($u16Selection.CapacityKind -cne "analogue" -or $u16Selection.OutputFrames -ne 128 -or $u16Selection.AlsaPeriodFrames -ne 32 -or $u16Selection.EngineBlockFrames -ne 32 -or $u16Selection.InternalFrames -ne 32 -or $u16Selection.LookaheadFrames -ne 0 -or $u16Selection.EffectiveOutputLatencyFrames -ne 128 -or $u16Selection.WorkerTimingMode -cne "disabled" -or $u16Selection.RequiredPoolStage -ne 128 -or $u16Selection.RequiredPoolCapacity -ne 72) { throw "Analogue inline capacity selection did not retain its exact geometry or pool stage." }
$u16Engine32Selection = $u16Selection
$u16Engine64Rejected = { Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_24" -OutputFrames 128 -EngineBlockFrames 64 -MeasureSeconds 30 -ExecutorMode "inline" -WorkerTimingMode "disabled" }
Assert-Throws $u16Engine64Rejected
...
$legacyOm64Capacity = Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_16" -OutputFrames 256 -EngineBlockFrames 64 -MeasureSeconds 120 -ExecutorMode "routing_tree_persistent" -WorkerTimingMode "disabled" -ContinueOnRecoveredMiss:$true
if ($legacyOm64Capacity.OutputFrames -ne 256 -or $legacyOm64Capacity.AlsaPeriodFrames -ne 64 -or $legacyOm64Capacity.InternalFrames -ne 64 -or $legacyOm64Capacity.LookaheadFrames -ne 64 -or $legacyOm64Capacity.EffectiveOutputLatencyFrames -ne 320 -or $legacyOm64Capacity.WorkerTimingMode -cne "disabled" -or -not $legacyOm64Capacity.ContinueOnRecoveredMiss -or $legacyOm64Capacity.RequiredPoolStage -ne 128) { throw "Legacy OM64 capacity selection did not retain output=256 period=64 internal=64 lookahead=64 with continuation." }
foreach ($parameters in @(
  ...
  foreach ($engineFrames in @(64)) {
    $invalidParameters = @{ Scenario = $parameters.Scenario; OutputFrames = 128; EngineBlockFrames = $engineFrames; MeasureSeconds = 30; ExecutorMode = $parameters.ExecutorMode; WorkerTimingMode = $parameters.WorkerTimingMode }
    Assert-Throws { Assert-OrangeLiveBenchmarkSelection @invalidParameters }
  }
}
Assert-Throws { Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_24" -OutputFrames 256 -EngineBlockFrames 64 -MeasureSeconds 30 -ExecutorMode "inline" -WorkerTimingMode "disabled" }
$routingOutput256 = Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_24" -OutputFrames 256 -EngineBlockFrames 64 -MeasureSeconds 30 -ExecutorMode "routing_tree_persistent" -WorkerTimingMode "disabled"
if ($routingOutput256.AlsaPeriodFrames -ne 64 -or $routingOutput256.LookaheadFrames -ne 64 -or $routingOutput256.EffectiveOutputLatencyFrames -ne 320) { throw "Analogue routing output=256 capacity geometry changed." }
...
foreach ($case in @(
  ...
  $selection = Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_$($case.Units)" -OutputFrames 256 -EngineBlockFrames 64 -MeasureSeconds 30 -ExecutorMode routing_tree_persistent -WorkerTimingMode disabled
...
$routingStage256 = New-DiagnosticFixture 256 $true
try {
  ...
  $exact42 = Invoke-StudyPrintOnly -Parameters @{ Mode = "LiveAudioBenchmark"; Scenario = "capacity_analogue_42"; OutputFrames = 256; EngineBlockFrames = 64; MeasureSeconds = 30; ExecutorMode = "routing_tree_persistent"; WorkerTimingMode = "disabled"; Artifact = $routingStage128.Binary; Metadata = $routingStage128.Metadata; AllowServiceInterruption = $true; PrintOnly = $true }
  Assert-Contains $exact42 "Live artifact identity: artifact_kind=diagnostic-only cargo_feature=hardware-orange-pi-zero-2w routing-tree-benchmark benchmark-voice-pools-128"
  ...
  $u16Engine32Payload = Invoke-StudyPrintOnly -Parameters @{ Mode = "LiveAudioBenchmark"; Scenario = "capacity_analogue_24"; OutputFrames = 128; EngineBlockFrames = 32; MeasureSeconds = 30; ExecutorMode = "inline"; WorkerTimingMode = "disabled"; Artifact = $stage128.Binary; Metadata = $stage128.Metadata; AllowServiceInterruption = $true; PrintOnly = $true }
  Assert-Contains $u16Engine32Payload "Live selection: diagnostic output=128 period=32 engine=32 internal=32 scenario=capacity_analogue_24 measure=30 warmup=5 worker-timing=disabled executor=inline lookahead=0 effective-latency=128"
  Assert-Contains $u16Engine32Payload "--output-frames 128 --engine-block-frames 32"
  Assert-Contains $u16Engine32Payload "Diagnostic pool identity: benchmark-voice-pools-128 requested-synth=72 requested-sample=24 required-capacity=72 required-pool-stage=128"
  foreach ($invalid in @(
      @{ Scenario = "capacity_analogue_24"; ExecutorMode = "persistent_two_workers"; WorkerTimingMode = "disabled" },
      @{ Scenario = "capacity_analogue_24"; ExecutorMode = "routing_tree_persistent"; WorkerTimingMode = "enabled" },
      @{ Scenario = "capacity_mixed_16_16"; ExecutorMode = "inline"; WorkerTimingMode = "disabled" },
  ...
  $exact43 = Invoke-StudyPrintOnly -Parameters @{ Mode = "LiveAudioBenchmark"; Scenario = "capacity_analogue_43"; OutputFrames = 256; EngineBlockFrames = 64; MeasureSeconds = 30; ExecutorMode = "routing_tree_persistent"; WorkerTimingMode = "disabled"; Artifact = $routingStage256.Binary; Metadata = $routingStage256.Metadata; AllowServiceInterruption = $true; PrintOnly = $true }
  ...
  $exact85 = Invoke-StudyPrintOnly -Parameters @{ Mode = "LiveAudioBenchmark"; Scenario = "capacity_analogue_85"; OutputFrames = 256; EngineBlockFrames = 64; MeasureSeconds = 30; ExecutorMode = "routing_tree_persistent"; WorkerTimingMode = "disabled"; Artifact = $routingStage256.Binary; Metadata = $routingStage256.Metadata; AllowServiceInterruption = $true; PrintOnly = $true }
...
$u16Result.schema_version = 13; $u16Result.internal_block_frames = 32; $u16Result | Add-Member -NotePropertyName continue_on_recovered_miss -NotePropertyValue $false; $u16Result | Add-Member -NotePropertyName persistent_output_provenance -NotePropertyValue ([pscustomobject]@{ observable = $false; repeated_quantum_incidents = 0; repeated_pcm_frames = 0; silent_quantum_incidents = 0; silent_pcm_frames = 0 })
$u16Result.schema_version = 13; $u16Result.requested_output_buffer_frames = 128; $u16Result.expected_alsa_buffer_frames = 128; $u16Result.expected_alsa_period_frames = 32; $u16Result.internal_block_frames = 32; $u16Result.sample_format = "U16"; $u16Result.executor_mode = "inline"; $u16Result | Add-Member -NotePropertyName lookahead_frames -NotePropertyValue 0; $u16Result | Add-Member -NotePropertyName effective_output_latency_frames -NotePropertyValue 128; $u16Result.worker_health = "disabled"; $u16Result.worker_thread_name_0 = ""; $u16Result.worker_thread_name_1 = ""; $u16Result.joined_workers = 0; $u16Result.worker_timing_mode = "disabled"; $u16Result.worker_timing = $null; $u16Result.persistent_output_counters = [pscustomobject]@{ observable = $false; warmup = $zeroU16Counters; start = $zeroU16Counters; end = $zeroU16Counters; delta = $zeroU16Counters }; $u16Result.detected_continuity_events = 0
Assert-OrangeLiveResult -Result $u16Result -Selection $u16Selection
$u16Engine32Result = ConvertFrom-Json -InputObject ($u16Result | ConvertTo-Json -Depth 8); $u16Engine32Result.internal_block_frames = 32
Assert-OrangeLiveResult -Result $u16Engine32Result -Selection $u16Engine32Selection
$u16Readiness = [pscustomobject]@{ schema_version = 5; kind = "orange_audio_benchmark_readiness"; status = "ready"; board_profile = "orange-pi-zero-2w"; pid = 123; systemd_invocation_id = "invocation"; artifact_sha256 = ("a" * 64); scenario = $u16Selection.Scenario; requested_output_buffer_frames = 128; expected_alsa_buffer_frames = 128; expected_alsa_period_frames = 32; internal_block_frames = 32; lookahead_frames = 0; callback_frames_min = 100; callback_frames_max = 100; callback_frame_sample_count = 3; callback_frame_size_change_count = 0; sample_rate = 44100; channels = 2; sample_format = "U16"; scheduler_qualified = $true; post_dsp_zero = $true; executor_mode = "inline"; worker_health = "disabled"; worker_thread_name_0 = ""; worker_thread_name_1 = "" }
$u16Readiness | Add-Member -NotePropertyName invalid_callback_frame_count -NotePropertyValue 0
...
$u16Engine32Readiness = ConvertFrom-Json -InputObject ($u16Readiness | ConvertTo-Json -Depth 8); $u16Engine32Readiness.internal_block_frames = 32
Assert-OrangeLiveReadiness -Readiness $u16Engine32Readiness -Selection $u16Engine32Selection -ExpectedPid 123 -ExpectedInvocation "invocation" -ArtifactHash ("a" * 64)
...
Assert-OrangeLiveRelease -Release $u16Release -Selection $u16Engine32Selection -ExpectedPid 123 -ExpectedInvocation "invocation" -ArtifactHash ("a" * 64)
