...
Import-Module (Join-Path $PSScriptRoot "orange-live-payload-validation.psm1") -Force; Import-Module (Join-Path $PSScriptRoot "orange-live-result-evidence-validation.psm1") -Global -Force; Import-Module (Join-Path $PSScriptRoot "orange-live-study-outcome-validation.psm1") -Force
...
try {
  ...
  $profileStart = [pscustomobject]@{ active_synth_voices = 0; active_sample_voices = 0; active_preview_sample_voices = 0; active_momentary_fx = 0; active_bus_fx_slots = 0; active_global_fx_slots = 0; cumulative_voice_steals = 0; cumulative_voice_admission_drops = 0 }
  $profileEnd = [pscustomobject]@{ active_synth_voices = 0; active_sample_voices = 0; active_preview_sample_voices = 0; active_momentary_fx = 0; active_bus_fx_slots = 0; active_global_fx_slots = 0; cumulative_voice_steals = 0; cumulative_voice_admission_drops = 0 }
  ...
  $zeroPersistentCounters = [pscustomobject]@{ rendered_quantums = 0; repeated_quantums = 0; dropped_quantums = 0; deadline_misses = 0; deadline_recoveries = 0 }
  $persistentOutputCounters = [pscustomobject]@{ observable = $true; warmup = [pscustomobject]@{ rendered_quantums = 3; repeated_quantums = 1; dropped_quantums = 0; deadline_misses = 1; deadline_recoveries = 1 }; start = [pscustomobject]@{ rendered_quantums = 5; repeated_quantums = 1; dropped_quantums = 1; deadline_misses = 2; deadline_recoveries = 1 }; end = [pscustomobject]@{ rendered_quantums = 7; repeated_quantums = 2; dropped_quantums = 3; deadline_misses = 4; deadline_recoveries = 4 }; delta = [pscustomobject]@{ rendered_quantums = 2; repeated_quantums = 1; dropped_quantums = 2; deadline_misses = 2; deadline_recoveries = 3 } }
  $result = [pscustomobject]@{ schema_version = 13; kind = "orange_audio_benchmark_result"; status = "pass"; board_profile = "orange-pi-zero-2w"; scenario = $selection.Scenario; requested_output_buffer_frames = 256; expected_alsa_buffer_frames = 256; expected_alsa_period_frames = 64; internal_block_frames = 256; sample_format = "F32"; channels = 2; sample_rate = 44100; warmup_seconds = 5; measure_seconds = 30; scheduler_qualified = $true; post_dsp_zero = $true; measurement_stop_acknowledged = $true; stream_stopped = $true; final_progress_write_succeeded = $true; pid = 123; systemd_invocation_id = "invocation"; artifact_sha256 = ("a" * 64); callback = $callback; persistent_output_counters = $persistentOutputCounters; detected_continuity_events = 3; profile_start = $profileStart; profile_end = $profileEnd; recovered_alsa_epipe_count = $null; recovered_alsa_epipe_observable = $false; terminal_error = $null; executor_mode = "persistent_two_workers"; worker_health = "healthy"; worker_thread_name_0 = "oct-dsp-src-0"; worker_thread_name_1 = "oct-dsp-src-1"; joined_workers = 2; retirement_error = $null; worker_timing_mode = "enabled"; worker_timing = $workerTiming }
  ...
  $result | Add-Member -NotePropertyName callback_scheduling_cpu -NotePropertyValue 1; $result | Add-Member -NotePropertyName lookahead_frames -NotePropertyValue 0; $result | Add-Member -NotePropertyName effective_output_latency_frames -NotePropertyValue 256
  ...
  $result | Add-Member -NotePropertyName persistent_output_provenance -NotePropertyValue ([pscustomobject]@{ observable = $true; repeated_quantum_incidents = 0; repeated_pcm_frames = 0; silent_quantum_incidents = 0; silent_pcm_frames = 0 }); $result | Add-Member -NotePropertyName continue_on_recovered_miss -NotePropertyValue $false; $result | ConvertTo-Json -Depth 8 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json")
  Set-Content (Join-Path $evidenceRoot "unit-journal.txt") ""; $passEvidence = Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64)
  ...
  if ($passEvidence.SensorStartupMaxThermalMillicelsius -ne 70000 -or $passEvidence.SensorRuntimeMaxThermalMillicelsius -ne 75000 -or $passEvidence.SensorMaxThermalMillicelsius -ne 75000 -or -not $passEvidence.SensorCoolingObserved -or $passEvidence.SensorMaxCoolingState -ne 3 -or $passEvidence.SensorMinFrequencyKhz -ne 800000 -or -not $passEvidence.SensorCoolingEvidenceValid) { throw "Thermal, cooling-state, or frequency sensor evidence was not retained." }; Remove-Item -LiteralPath (Join-Path $evidenceRoot "unit-journal.txt") -Force; $missingJournal = Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64); if ($missingJournal.StatusClass -ne "infrastructure_failure" -or $missingJournal.Reason -notlike "*unit-journal.txt*") { throw "Missing validated unit journal was not classified as infrastructure failure." }; Set-Content (Join-Path $evidenceRoot "unit-journal.txt") ""; $emptyJournal = Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64); if ($emptyJournal.StatusClass -ne "pass" -or $emptyJournal.PracticalGrade -ne "Stable" -or $emptyJournal.AlsaRecoveryLogIncidents -ne 0) { throw "An empty validated unit journal was not treated as zero incidents." }
  Set-Content (Join-Path $evidenceRoot "unit-journal.txt") @("snd_pcm_recover: underrun occurred", "snd_pcm_recover(0x1): underrun occurred", "EPIPE without recovery", "snd_pcm_recover without an underrun"); $passEvidence = Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64); if ($passEvidence.AlsaRecoveryLogIncidents -ne 2 -or $passEvidence.AlsaRecoveryLogScope -cne "whole_run_unit_journal" -or $passEvidence.PracticalGrade -cne "Stretched") { throw "ALSA recovery evidence or practical grade was incorrect." }
  $result.persistent_output_provenance.repeated_quantum_incidents = 5; $result.persistent_output_provenance.repeated_pcm_frames = 5; $result.status = "fail"; $result.worker_health = "healthy"; $result | ConvertTo-Json -Depth 8 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json"); Set-Content (Join-Path $evidenceRoot "unit-final.txt") "ActiveState=failed`nSubState=dead`nResult=exit-code`nMainPID=0`nExecMainCode=1`nExecMainStatus=1"; Set-Content (Join-Path $evidenceRoot "study-result.txt") "interruption_started=true`nstatus_class=measured_failure"
  $compromised = Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64); if ($compromised.StatusClass -ne "measured_failure" -or $compromised.PracticalGrade -ne "Compromised" -or $compromised.BenchmarkOutcome -ne "fail" -or $result.callback.callback_count -ne 441) { throw "A completed compromised observation was stopped or graded as a failure." }
  $result.persistent_output_provenance.repeated_quantum_incidents = 0; $result.persistent_output_provenance.repeated_pcm_frames = 0; $result.status = "pass"; $result | ConvertTo-Json -Depth 8 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json"); Set-Content (Join-Path $evidenceRoot "unit-final.txt") "ActiveState=inactive`nSubState=dead`nResult=success`nMainPID=0`nExecMainCode=1`nExecMainStatus=0"; Set-Content (Join-Path $evidenceRoot "study-result.txt") "interruption_started=true`nstatus_class=pass"
  ...
  foreach ($durationCase in @(
      ...
      @{ Selection = (Assert-OrangeLiveBenchmarkSelection -Scenario "synth_cross_slot_96_steal" -OutputFrames 256 -EngineBlockFrames 128 -MeasureSeconds 180); Allowed = 0; Rejected = 1 },
  ...
  $invalidPersistentShape = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentShape.persistent_output_counters.PSObject.Properties.Remove("delta"); Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentShape -Selection $selection }
  $invalidPersistentType = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentType.persistent_output_counters.warmup.rendered_quantums = "3"; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentType -Selection $selection }
  $invalidPersistentNegative = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentNegative.persistent_output_counters.end.dropped_quantums = -1; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentNegative -Selection $selection }
  $invalidPersistentUnknown = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentUnknown.persistent_output_counters.delta | Add-Member -NotePropertyName unknown -NotePropertyValue 1; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentUnknown -Selection $selection }; $invalidPersistentCase = ConvertFrom-Json -InputObject (($result | ConvertTo-Json -Depth 8) -replace '"rendered_quantums"', '"Rendered_Quantums"'); Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentCase -Selection $selection }
  $invalidPersistentObservability = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentObservability.persistent_output_counters.observable = $false; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentObservability -Selection $selection }; $invalidPersistentObservability.persistent_output_counters.observable = "true"; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentObservability -Selection $selection }
  $invalidPersistentMonotonic = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentMonotonic.persistent_output_counters.start.rendered_quantums = 2; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentMonotonic -Selection $selection }; $invalidProvenance = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidProvenance.persistent_output_provenance.repeated_quantum_incidents = 1; Assert-Throws { Assert-OrangeLiveResult -Result $invalidProvenance -Selection $selection }; $invalidProvenance = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidProvenance.persistent_output_provenance.repeated_quantum_incidents = 2; $invalidProvenance.persistent_output_provenance.repeated_pcm_frames = 1; Assert-Throws { Assert-OrangeLiveResult -Result $invalidProvenance -Selection $selection }; $invalidProvenance = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidProvenance.persistent_output_provenance.silent_pcm_frames = 1; Assert-Throws { Assert-OrangeLiveResult -Result $invalidProvenance -Selection $selection }
  $invalidPersistentDelta = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentDelta.persistent_output_counters.delta.rendered_quantums = 1; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentDelta -Selection $selection }
  $invalidPersistentRecovery = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentRecovery.persistent_output_counters.end.deadline_recoveries = 5; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentRecovery -Selection $selection }; $invalidDeltaRecovery = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidDeltaRecovery.persistent_output_counters.start.deadline_misses = 3; $invalidDeltaRecovery.persistent_output_counters.start.dropped_quantums = 2; $invalidDeltaRecovery.persistent_output_counters.delta.deadline_misses = 1; $invalidDeltaRecovery.persistent_output_counters.delta.deadline_recoveries = 3; Assert-Throws { Assert-OrangeLiveResult -Result $invalidDeltaRecovery -Selection $selection }
  $invalidPersistentDisposition = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentDisposition.persistent_output_counters.end.repeated_quantums = 5; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentDisposition -Selection $selection }
  $invalidPersistentMisses = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidPersistentMisses.persistent_output_counters.end.deadline_misses = 6; Assert-Throws { Assert-OrangeLiveResult -Result $invalidPersistentMisses -Selection $selection }
  $invalidContinuity = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $invalidContinuity.detected_continuity_events = 2; Assert-Throws { Assert-OrangeLiveResult -Result $invalidContinuity -Selection $selection }
  $noCarryIn = ConvertFrom-Json -InputObject ($result | ConvertTo-Json -Depth 8); $noCarryIn.persistent_output_counters.start.deadline_recoveries = 2; $noCarryIn.persistent_output_counters.delta.deadline_recoveries = 2; $noCarryIn.detected_continuity_events = 2; Assert-OrangeLiveResult -Result $noCarryIn -Selection $selection
  ...
  $inlineFailure.persistent_output_provenance.observable = $false; $inlineFailure.persistent_output_provenance.repeated_quantum_incidents = 0; $inlineFailure.persistent_output_provenance.repeated_pcm_frames = 0; $inlineFailure.persistent_output_provenance.silent_quantum_incidents = 0; $inlineFailure.persistent_output_provenance.silent_pcm_frames = 0; $inlineFailure.continue_on_recovered_miss = $false; $inlineFailure | ConvertTo-Json -Depth 8 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json")
  ...
  $callback.rendered_frames = 0; $result | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json"); if ((Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64)).StatusClass -ne "infrastructure_failure") { throw "Zero rendered-frame aggregate evidence was accepted." }
  $callback.rendered_frames = 44100; $callback.render_audio_duration_ns = 0; $result | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json"); if ((Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64)).StatusClass -ne "infrastructure_failure") { throw "Zero render-duration aggregate evidence was accepted." }
  ...
  $callback.cpal_device_error_count = 1; $result | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json"); if ((Get-OrangeLiveHostEvidence $evidenceRoot $selection ("a" * 64)).StatusClass -ne "infrastructure_failure") { throw "Pass with CPAL error was accepted." }
  $callback.cpal_device_error_count = 0; $callback.over_audio_duration_budget_count = 6; $result.detected_continuity_events = 6
  $result | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json")
  ...
  $callback.over_audio_duration_budget_count = 0; $result.detected_continuity_events = 3; $callback.pre_mute_nonzero_samples = 0
  $result | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json")
  ...
  $release | ConvertTo-Json | Set-Content (Join-Path $evidenceRoot "benchmark-release.json")
  $callback.over_audio_duration_budget_count = 6; $result.detected_continuity_events = 6
  $result.status = "fail"
  Set-Content (Join-Path $evidenceRoot "study-result.txt") "interruption_started=true`nstatus_class=measured_failure"
  $result | ConvertTo-Json -Depth 4 | Set-Content (Join-Path $evidenceRoot "benchmark-result.json")
...
if ((Resolve-OrangeLiveRunnerOutcome "pass" $false) -ne "pass" -or (Resolve-OrangeLiveRunnerOutcome "pass" $true) -ne "infrastructure_failure" -or (Resolve-OrangeLiveRunnerOutcome "over_budget" $true) -ne "over_budget" -or (Resolve-OrangeLiveRunnerOutcome "measured_failure" $true) -ne "measured_failure" -or (Resolve-OrangeLiveRunnerOutcome "restoration_failure" $true) -ne "restoration_failure") { throw "Runner outcome classification changed incorrectly." }; $completedSelection = Assert-OrangeLiveBenchmarkSelection -Scenario "capacity_analogue_36" -OutputFrames 256 -EngineBlockFrames 64 -MeasureSeconds 120 -ExecutorMode routing_tree_persistent -WorkerTimingMode disabled -ContinueOnRecoveredMiss:$true; Assert-OrangeLiveStudyOutcome -Selection $completedSelection -HostStatusClass measured_failure -HostReason "completed compromised observation" -StudyFailure "Orange transport failed with exit code 20: ssh-payload" -ContinueOnRecoveredMiss:$true; Assert-OrangeLiveStudyOutcome -Selection $completedSelection -HostStatusClass over_budget -HostReason "completed over-budget observation" -StudyFailure "Orange transport failed with exit code 20: ssh-payload" -ContinueOnRecoveredMiss:$true; Assert-Throws { Assert-OrangeLiveStudyOutcome -Selection $completedSelection -HostStatusClass measured_failure -HostReason "missing expected transport failure" -ContinueOnRecoveredMiss:$true }; Assert-Throws { Assert-OrangeLiveStudyOutcome -Selection $completedSelection -HostStatusClass infrastructure_failure -HostReason "identity validation failed" -ContinueOnRecoveredMiss:$true }; Assert-Throws { Assert-OrangeLiveStudyOutcome -Selection $completedSelection -HostStatusClass measured_failure -HostReason "other transport failure" -StudyFailure "Orange transport failed with exit code 1: ssh-payload" -ContinueOnRecoveredMiss:$true }; Import-Module (Join-Path $PSScriptRoot "orange-live-result-evidence-validation.psm1") -Global -Force; if ((Get-OrangeLivePracticalGrade -RepeatIncidents 0 -SilentIncidents 0 -AlsaRecoveryLogIncidents 0 -CpalStreamErrors 3 -CpalDeviceErrors 3) -ne "Stretched") { throw "Separate CPAL error categories were summed instead of graded by their maximum." }
...
($captureFixture -replace "`r", "") | & bash -s
