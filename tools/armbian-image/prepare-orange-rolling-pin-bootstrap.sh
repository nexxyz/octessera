...
(
  cd -- "$build_dir" || exit 1
  ./compile.sh artifact-config-dump-json WHAT=kernel BOARD=orangepizero2w RELEASE=trixie BRANCH=current REVISION=26.11.0-trunk.22 HOSTRELEASE=trixie HOSTARCH=amd64 BUILD_DESKTOP=no BUILD_MINIMAL=yes KERNEL_CONFIGURE=no ENABLE_EXTENSIONS='octessera_midi octessera_audio octessera_sd2 octessera_image_sanitize' EXPERT=yes
)
