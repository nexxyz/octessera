...
def run_runtime_proof(work: Path, image: Path, dtb: Path, evidence: Path, provenance: Path, manifest: Path) -> None:
    ...
    write(production / "etc/login.defs", "# Armbian login defaults\nTTYPERM 0620\nUID_MIN 1000\n")
    ...
    if can_privilege:
        try:
            ...
            for name, mutate in (
                ...
                ("missing-input-group", lambda root: (root / "etc/group").write_text((root / "etc/group").read_text().replace("input:x:104:\n", ""))),
            ):
                negative = work / name
            ...
            for name, mutate, reason in (
                ...
                ("wrong-firstrun-enable", lambda root: ((root / FIRSTRUN_ENABLE_RELATIVE).unlink(), (root / FIRSTRUN_ENABLE_RELATIVE).symlink_to("../../../lib/systemd/system/armbian-firstrun.service")), "Orange Armbian firstrun service is not enabled"),
