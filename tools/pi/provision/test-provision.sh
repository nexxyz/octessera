...
# 4. First run installs payload, cleans UART, and requests reboot (exit 75).
new_fixture
run_provision 0
expect_rc "sc4" 75
...
pass "first run installs payload and UART cleanup, exits 75"
...
# 6. Initramfs handling installs hooks and calls update-initramfs.
new_fixture
run_provision 1
expect_rc "sc6" 75
...
assert_mode "sc6" "$FIXTURE/etc/initramfs-tools/hooks/octessera-boot-splash" 755
...
pass "initramfs handling installs hooks and invokes update-initramfs"

# 7. Reboot-required via unsafe GPIO state.
new_fixture
run_provision 0
expect_rc "sc7a" 75
FAKE_PINCTRL_OUTPUT=unsafe run_provision 0
expect_rc "sc7" 75
