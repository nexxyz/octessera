...
function Assert-OfflineCell {
  ...
  Assert-PlanFields $Cell @("id", "scenario", "internal_frames", "measure_frames") $Context
...
function Assert-OrangeLiveCell {
  param([object]$Cell, [string]$Context)
  Assert-PlanFields $Cell @("id", "scenario", "output_frames", "internal_frames", "measure_seconds") $Context
  Assert-PlanString $Cell.id "$Context.id"
  if ($Cell.id -notmatch '^[a-z0-9][a-z0-9_]*$') { throw "$Context.id is not a canonical cell ID." }
  if ($script:OrangeLiveIds -notcontains $Cell.scenario) { throw "$Context has an unknown Orange live scenario ID: $($Cell.scenario)." }
  foreach ($field in @("output_frames", "internal_frames", "measure_seconds")) { Assert-PlanInteger $Cell.$field "$Context.$field" }
  $tuple = "$($Cell.output_frames)/$($Cell.internal_frames)"
  if (@("128/32", "256/64", "256/128", "256/256", "512/128", "1024/256") -notcontains $tuple) { throw "$Context has invalid Orange live geometry: $tuple." }
...
function Assert-PlanTuple {
  param([object]$Cell, [int]$OutputFrames, [int]$InternalFrames, [int]$MeasureFrames, [string]$Context)
  $output = $Cell.PSObject.Properties["output_frames"]
  $measure = $Cell.PSObject.Properties["measure_frames"]
  if (($null -ne $output -and $Cell.output_frames -ne $OutputFrames) -or $Cell.internal_frames -ne $InternalFrames -or ($null -ne $measure -and $Cell.measure_frames -ne $MeasureFrames)) { throw "$Context has an unexpected geometry tuple." }
}
...
function Read-PerformanceBaselineManifest {
  ...
  if ($manifest.schema_version -ne 2 -or $manifest.study_id -cne "cross-board-baseline" -or $manifest.sample_rate -ne 44100 -or $manifest.warmup_seconds -ne 2 -or $manifest.offline_observations -ne 4096 -or $manifest.repetitions -ne 3) { throw "Performance baseline manifest has an unsupported study contract." }

  Assert-PlanFields $manifest.cohorts @("common_reference", "orange_effective_default", "block_sweep") "manifest.cohorts"
  $cohortCells = [ordered]@{}
  foreach ($cohortName in @("common_reference", "orange_effective_default", "block_sweep")) {
    $cohort = $manifest.cohorts.$cohortName
    $cohortFields = if ($cohortName -eq "block_sweep") { @("kind", "scenarios", "cells") } else { @("kind", "cells") }
    ...
    if ($cohortName -eq "block_sweep") {
      if ($cohort.scenarios -isnot [array]) { throw "manifest.cohorts.$cohortName.scenarios must be an array." }
      if (($cohort.scenarios -join ",") -cne ($script:BlockIds -join ",")) { throw "manifest.cohorts.$cohortName scenario order is invalid." }
    }
    $cohortCells[$cohortName] = $cells
  }
  if ($cohortCells.common_reference.Count -ne 12 -or $cohortCells.orange_effective_default.Count -ne 10 -or $cohortCells.block_sweep.Count -ne 9) { throw "Performance baseline cohort counts do not match the explicit study." }
  ...
  foreach ($cell in $cohortCells.common_reference) { Assert-PlanTuple $cell 0 256 256 "common_reference.$($cell.id)" }
  foreach ($cell in $cohortCells.orange_effective_default) { Assert-PlanTuple $cell 0 128 128 "orange_effective_default.$($cell.id)" }
  for ($index = 0; $index -lt $cohortCells.block_sweep.Count; $index++) { $blockFrames = @(64, 128, 256)[$index % 3]; Assert-PlanTuple $cohortCells.block_sweep[$index] 0 $blockFrames $blockFrames "block_sweep.$($cohortCells.block_sweep[$index].id)" }
  Assert-UniquePlanIds @($cohortCells.Values | ForEach-Object { $_ }) "native cohorts"

  Assert-PlanFields $manifest.orange @("board_profile", "live_defaults", "live_neighbors", "long_repeat") "manifest.orange"
  if ($manifest.orange.board_profile -cne "orange-pi-zero-2w") { throw "manifest.orange has the wrong board profile." }
  foreach ($groupName in @("live_defaults", "live_neighbors")) {
  ...
  if (@($manifest.orange.live_defaults).Count -ne 4 -or @($manifest.orange.live_neighbors).Count -ne 4) { throw "Orange live cohort counts do not match the explicit study." }
  ...
  foreach ($cell in @($manifest.orange.live_defaults)) { Assert-PlanTuple $cell 256 128 0 "orange.live_defaults.$($cell.id)" }
  foreach ($cell in @($manifest.orange.live_neighbors)) { $neighborIndex = [array]::IndexOf(@($manifest.orange.live_neighbors), $cell); $tuple = if ($neighborIndex % 2 -eq 0) { @(128, 32) } else { @(512, 128) }; Assert-PlanTuple $cell $tuple[0] $tuple[1] 0 "orange.live_neighbors.$($cell.id)" }
  Assert-UniquePlanIds @($manifest.orange.live_defaults) + @($manifest.orange.live_neighbors) "Orange live cohorts"
  ...
  foreach ($cell in @($manifest.raspberry.live_cells)) {
    Assert-PlanFields $cell @("id", "scenario", "output_frames", "internal_frames", "duration_seconds", "probe_modes", "callback_fields") "manifest.raspberry.$($cell.id)"
