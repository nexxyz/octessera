...
function Assert-DeploymentTarget {
  ...
  if ([string]::IsNullOrEmpty($Target)) {
    throw "Deployment target must be a non-empty user@IPv4 or user@hostname value."
  }

  foreach ($character in $Target.ToCharArray()) {
    if ([char]::IsControl($character) -or [char]::IsWhiteSpace($character)) {
      throw "Deployment target must not contain control characters or whitespace."
    }
  }
  if ($Target[0] -eq '-') {
    throw "Deployment target must not begin with a host option."
  }

  $atIndex = $Target.IndexOf('@')
  if ($atIndex -le 0 -or $atIndex -ne $Target.LastIndexOf('@')) {
    throw "Deployment target must contain exactly one user@host separator."
  }

  $username = $Target.Substring(0, $atIndex)
  $hostname = $Target.Substring($atIndex + 1)
  if ($username -notmatch '^[A-Za-z_][A-Za-z0-9_.-]*$') {
    throw "Deployment target contains an unsafe username."
  }
  if ([string]::IsNullOrEmpty($hostname) -or $hostname[0] -eq '-') {
    throw "Deployment target contains an invalid host."
  }
  if ($hostname.Length -gt 253) {
    throw "Deployment target host is too long."
  }

  if ($hostname -match '^[0-9.]+$') {
    if ($hostname -notmatch '^\d+\.\d+\.\d+\.\d+$') {
      throw "Deployment target contains a malformed IPv4 address."
    }
  ...
  if ($hostname -notmatch "^(?:$label)(?:\.(?:$label))*$") {
    throw "Deployment target contains a malformed hostname."
  }
  $Target
}
