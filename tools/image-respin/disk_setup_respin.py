...
try:
    ...
    from .disk_packaging import DiskPackagingError, compression_identity, file_digest, package_derived, prepare_parent_image, provenance_sidecar, verify_parent_asset, write_derived_sidecar
...
def _setup_provenance(runtime: MutationResult, setup: SetupMutationResult, proof: dict[str, Any], image: Path, packaged: Path, pre_layout: Any, post_layout: Any, parent_context: dict[str, Any], manifest_digest: str, source_identity: object, version: str, boot_integrity: dict[str, Any] | None = None, boot_policy: dict[str, Any] | None = None, boot_parent: dict[str, Any] | None = None) -> dict[str, Any]:
    if boot_integrity is not None:
        return build_derived_provenance(board_profile=setup.board_profile, version=version, source_identity=source_identity, parent_context=parent_context, trust_manifest_digest=manifest_digest, runtime_provenance=runtime.provenance, pre_layout=pre_layout, post_layout=post_layout, image=image, packaged=packaged, compression_identity=compression_identity(setup.board_profile), tool_identity=TOOL_IDENTITY, boot_integrity=boot_integrity, boot_policy=boot_policy, parent_binding=boot_parent, derivation_kind="setup-portal", setup_mutation={"digest": digest_object(setup.provenance), "provenance": setup.provenance}, setup_proof=proof)
    image_digest, image_size = file_digest(image)
    package_digest, package_size = file_digest(packaged)
    image_identity = {"sha256": image_digest, "size": image_size}
    package_identity = {"sha256": package_digest, "size": package_size, "path": packaged.name}
    setup_tool = setup_tool_code_model()
    return {"proof_schema": "octessera.image-derived-setup-respin-provenance.v1", "schema_version": 1, "board_profile": setup.board_profile, "version": version, "source_identity": source_identity, "parent": {"context": parent_context, "trust_manifest_sha256": manifest_digest, "digest": digest_object({"context": parent_context, "trust_manifest_sha256": manifest_digest})}, "runtime_mutation": {"digest": digest_object(runtime.provenance), "provenance": runtime.provenance}, "setup_mutation": {"digest": digest_object(setup.provenance), "provenance": setup.provenance}, "setup_proof": proof, "disk_invariants": {"pre": pre_layout.as_dict(), "post": post_layout.as_dict(), "digest": digest_object({"pre": pre_layout.as_dict(), "post": post_layout.as_dict()})}, "derived_image": image_identity, "packaged_artifact": package_identity, "finalizer": {"tool_identity": TOOL_IDENTITY, "compression_identity": compression_identity(setup.board_profile), "setup_tool_code": setup_tool}}


def respin_setup_image(*, board_profile: str, assets_directory: Path, manifest_path: Path, runtime_bundle: Path, version: str, source_identity: object, output: Path, proof_output: Path, boot_neutral_contract: Path | None = None, mutation_hook: Callable[[str], None] | None = None) -> dict[str, Any]:
    require_linux_root()
    ...
    if board_profile == "orange-pi-zero-2w":
        ...
        raise DiskSetupRespinError("boot-neutral policy is Orange-only")
    ...
    try:
        ...
        provenance = _setup_provenance(runtime, setup, proof, prepared.image, package, mounted.pre_layout, mounted.post_layout, parent_context, manifest_digest, source_identity, version, boot_integrity, boot_policy, boot_parent)
...
def main() -> int:
    ...
    result = respin_setup_image(board_profile=args.board, assets_directory=args.assets_directory, manifest_path=args.manifest, runtime_bundle=args.runtime_bundle, version=args.version, source_identity=args.source_identity, output=args.output, proof_output=args.proof_output, boot_neutral_contract=args.boot_neutral_contract)
