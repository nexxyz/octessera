...
def build_derived_provenance(
    *,
    ...
    if boot_integrity is None or boot_policy is None or parent_binding is None or derivation_kind not in {"runtime-only", "setup-portal"}:
        raise ValueError("Orange boot-neutral provenance context is incomplete")
    if parent_binding.get("context") != parent_context or parent_binding.get("record", {}).get("sha256") != parent_record_digest:
        raise ValueError("Orange current parent binding is inconsistent")
