...
PROOF_TEMPLATE = {
    ...
    "raspberry-sanitized": ("raspberry-sanitized", "sudo bash tools/pi-image/verify-sanitized-image.sh {artifact} 2>&1 | tee {proof_output} >/dev/null", RPI + "/sanitized-image-proof/v1"),
    "raspberry-kernel": ("raspberry-kernel", "sudo bash tools/pi-image/verify-rpi-kernel-image.sh --image {extracted_image} --package parent-assets/linux-image-6.12.93-octessera-rpi-v8-0.7.5_6.12.93-octessera0.7.5-1_arm64.deb --checksum parent-assets/octessera-0.7.5-raspberry-pi-zero-2w-kernel-SHA256SUMS --provenance parent-assets/octessera-0.7.5-raspberry-pi-zero-2w-kernel-provenance.json --manifest tools/kernel-patches/orange-midi-interface-manifest.json 2>&1 | tee {proof_output} >/dev/null", RPI + "/kernel-image-proof/v1"),
}
