...
try:
    ...
    from .disk_packaging import DiskPackagingError, compression_identity, package_derived, provenance_sidecar, verify_current_parent_asset, write_derived_sidecar, prepare_parent_image
...
def respin_image(
    *,
    ...
    if board_profile != "orange-pi-zero-2w":
        raise DiskRespinError(f"no current parent record exists for {board_profile}")
    ...
    try:
        ...
        provenance = build_derived_provenance(board_profile=board_profile, version=version, source_identity=source_identity, parent_context=parent_context, parent_record_digest=parent_record_digest, runtime_provenance=runtime_result.provenance, pre_layout=mounted.pre_layout, post_layout=mounted.post_layout, image=prepared.image, packaged=package, compression_identity=compression_identity(board_profile), tool_identity=TOOL_IDENTITY, boot_integrity=boot_integrity, boot_policy=boot_policy, parent_binding=boot_parent, derivation_kind="runtime-only")
...
def main() -> int:
    args = _arguments()
    result = respin_image(board_profile=args.board, assets_directory=args.assets_directory, parent_record_path=args.parent_record, runtime_bundle=args.runtime_bundle, version=args.version, source_identity=args.source_identity, output=args.output, boot_neutral_contract=args.boot_neutral_contract)
