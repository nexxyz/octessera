...
def load_policy(repository_root: Path, path: Path | None = None) -> BootNeutralPolicy:
    ...
    _require({"lib", "usr/lib/modules/6.18.46-current-sunxi64/modules.dep", "usr/lib/systemd/system-sleep/octessera-orange-oled"} <= set(paths) and "lib/modules/6.18.46-current-sunxi64/modules.dep" not in paths and "lib/systemd/system-sleep/octessera-orange-oled" not in paths, "Orange merged-/usr protected paths are not canonical")
