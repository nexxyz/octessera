...
try:
    from .current_parent import parent_context as current_parent_context
...
PROOF_MODE = "validated-parent"
CONTRACT_RELATIVE = "resources/image-derivations/boot-neutral/orange-pi-zero-2w-v0.8.1.json"
...
def load_policy(repository_root: Path, path: Path | None = None) -> BootNeutralPolicy:
    ...
    top = _exact(contract, {"schema", "schema_version", "proof_mode", "board_profile", "parent_record", "allowed_derivations", "mutation_authority", "boot_mutation", "protected_scopes", "protected_paths", "expected_absent_paths", "selected_boot", "respin_provenance", "setup_proof", "proofs"}, "Orange boot-neutral contract")
    ...
    _require(top["parent_record"] == "resources/image-parents/orange-pi-zero-2w-current.json" and top["mutation_authority"] == "none" and top["boot_mutation"] is False, "Orange boot-neutral policy changed")
    ...
    _require(module_scopes == [{"name": "kernel-modules-tree", "prefix": "usr/lib/modules/6.18.46-current-sunxi64", "kind": "recursive"}], "Orange merged-/usr module scope is not canonical")
    ...
    _require({"lib", "usr/lib/modules/6.18.46-current-sunxi64/modules.dep", "etc/systemd/system/octessera-orange-oled-suspend.service", "etc/systemd/system/sleep.target.requires/octessera-orange-oled-suspend.service", "usr/local/sbin/octessera-orange-oled-suspend", "usr/local/sbin/octessera-orange-oled-handoff.py"} <= set(paths) and not {"lib/systemd/system-sleep/octessera-orange-oled", "usr/lib/systemd/system-sleep/octessera-orange-oled"} & set(paths), "Orange OLED protected paths are not canonical")
    ...
    _require(selected == {"kernel_release": "6.18.46-current-sunxi64", "dtb_name": "sun50i-h618-orangepi-zero2w.dtb", "filesystem_types": ["ext4"], "partition_count": 1, "raw_boot_partition_sha256": None}, "Orange selected boot policy changed")
    ...
    _require(provenance["setup_top_level_additions"] == ["setup_mutation", "setup_proof"] and provenance["policy_keys"] == ["name", "version", "mutation_authority", "parent_finalization"] and provenance["boot_integrity_keys"] == ["pre", "post", "selected_kernel", "selected_initramfs", "selected_dtb", "selectors", "protected_scopes", "protected_paths", "expected_absent_paths", "changed_paths"] and provenance["inventory_keys"] == ["digest", "count"] and provenance["selector_keys"] == ["format", "kernel", "initramfs", "dtb"], "Orange provenance field policy changed")
    _require(provenance["policy"] == {"name": PROOF_MODE, "version": 1, "mutation_authority": "none", "parent_finalization": "forbidden"}, "Orange provenance policy identity changed")
...
def parent_binding(policy: BootNeutralPolicy, record_path: Path, record_digest: str, parent_context: dict[str, Any]) -> dict[str, Any]:
    expected_record = policy.root / policy.contract["parent_record"]
    _require(Path(record_path).resolve(strict=True) == expected_record.resolve(strict=True), "Orange current parent record path is not canonical")
    _require(Path(record_path).is_file() and not Path(record_path).is_symlink() and not expected_record.is_symlink(), "Orange current parent record is not a canonical regular file")
    current = current_parent_context(policy.root, expected_record)
    _require(current["record"]["sha256"] == record_digest and parent_context == current, "Orange current parent context changed")
    return {"record": dict(current["record"]), "context": current, "image": dict(current["image"]), "digest": _digest_parent(current, record_digest)}


def _digest_parent(context: dict[str, Any], record_digest: str) -> str:
    payload = json.dumps({"context": context, "record": {"path": context["record"]["path"], "sha256": record_digest, "size": context["record"]["size"]}, "image": context["image"]}, sort_keys=True, separators=(",", ":"), ensure_ascii=True).encode("utf-8")
