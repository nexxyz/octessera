...
class RespinWorkflowStaticTests(unittest.TestCase):
    ...
    def test_setup_layer_is_opt_in_and_separate_from_runtime_only(self) -> None:
        ...
        self.assertIn("verify-sanitized-image.sh --verification-profile legacy-setup-layer", self.text)
        self.assertIn("verify-sanitized-image.sh --verification-profile legacy-runtime-only", self.text)
