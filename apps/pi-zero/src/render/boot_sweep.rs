...
pub(crate) const BOOT_SWEEP_FRAMES: usize = 24;
pub(crate) const BOOT_SWEEP_CYCLE_NS: u64 = 1_000_000_000;
pub(crate) const BOOT_SWEEP_BAND_WIDTH: i32 = 8;
pub(crate) const BOOT_SWEEP_TRAIN_WIDTH: i32 = 32;
pub(crate) const BOOT_SWEEP_LEAN_NUMERATOR: i32 = 8;
pub(crate) const BOOT_SWEEP_LEAN_DENOMINATOR: i32 = 127;
pub(crate) const BOOT_SWEEP_COLORS: [u16; 4] = [0x07FF, 0xFFE0, 0x07E0, 0xF81F];
...
pub(crate) fn render_boot_splash(oled: &mut OledSsd1351) -> Result<(), String> {
    ...
    for frame_index in 0..BOOT_SWEEP_FRAMES {
        ...
        render_boot_splash_frame(oled, frame_index)?;
    }
    sleep_until(cycle_start + Duration::from_secs(1));
    Ok(())
}

pub(crate) fn render_boot_splash_frame(
    oled: &mut OledSsd1351,
    frame_index: usize,
) -> Result<(), String> {
    oled.write_frame(&boot_sweep_input_frame(frame_index))
}
...
pub(crate) fn boot_sweep_bottom_row_origin(frame_index: usize) -> i32 {
    -40 + (frame_index.min(BOOT_SWEEP_FRAMES - 1) as i32 * 168 / 23)
}
...
pub(crate) fn boot_sweep_frame_from(source: &[u8], frame_index: usize) -> Vec<u8> {
    ...
    for y in 0..128_i32 {
        for x in 0..128_i32 {
            ...
            {
                let color = BOOT_SWEEP_COLORS[(local_x / BOOT_SWEEP_BAND_WIDTH) as usize];
...
fn boot_sweep_input_frame(frame_index: usize) -> Vec<u8> {
