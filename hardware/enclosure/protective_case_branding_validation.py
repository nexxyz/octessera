...
def validate_deep_multicolor_geometry(params: dict, body: cq.Workplane, parts: list[tuple[str, cq.Workplane]]) -> None:
    expected_names = ["protective_case_deep_tub_logo", "protective_case_deep_tub_device_orientation_guide"]
...
def validate_branding(params: dict) -> None:
    ...
    for name, actual_parts, expected_parts, body, z_min, z_max in face_sets:
        ...
        expected_names = [f"protective_case_{owner_name}_{mark_name.removeprefix('octessera_')}" for mark_name, _ in expected_parts]
        ...
        for (actual_name, actual), (_, expected) in zip(actual_parts, expected_parts):
            ...
            reported_boxes[f"{name}_{actual_name.removeprefix('protective_case_')}"] = actual_box
