...
try:
    ...
    from .protective_case_device_fit import (
        ...
        build_canonical_face_down_device,
        support_contacts,
        validate_actual_device_fit,
    )
    ...
    from protective_case_corner_restraints import corner_restraint_components, validate_corner_restraints
    from protective_case_device_fit import EXPECTED_CONTACTS, _insertion_sweep_offsets, build_canonical_face_down_device, support_contacts, validate_actual_device_fit
    from protective_case_geometry import dimensions, face_down_transport_feature_xy, face_down_transport_xy, load_parameters, load_source_parameters, local_to_case
    from protective_case_mating_interface import apply_shallow_mating_interface
    from protective_case_keepouts import control_contact_keepouts
    from validate_protective_case import intersection_volume, prism, upstream_outer_envelope
...
class ProtectiveCaseDeviceFitTests(unittest.TestCase):
    ...
    def test_face_down_feature_transform_uses_rotated_case_xy_owner(self) -> None:
        ...
        self.assertAlmostEqual(face_down_transport_feature_xy(source, point, -0.5, -0.3)[0], 179.965)
        self.assertAlmostEqual(face_down_transport_feature_xy(source, point, -0.5, -0.3)[1], 114.9)
        key = source["features_local"]["neokey_key_centers"][0]
        self.assertAlmostEqual(face_down_transport_feature_xy(source, key, -0.25, -1.0)[0], 147.5)
        self.assertAlmostEqual(face_down_transport_feature_xy(source, key, -0.25, -1.0)[1], 78.075)
        self.assertAlmostEqual(face_down_transport_xy((75.635, 33.5))[0], 179.965)
        self.assertAlmostEqual(face_down_transport_xy((75.635, 33.5))[1], 114.9)

    def test_canonical_source_device_has_expected_planar_contacts(self) -> None:
        contacts = support_contacts(self.device, self.params)
    ...
    def test_actual_device_validator_qualifies_final_fit(self) -> None:
        validate_actual_device_fit(self.params)
    ...
    def test_reversed_shelf_elevations_report_penetration_and_gap(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, r"NW penetration=5\.000mm.*NE gap=5\.000mm"):
            validate_actual_device_fit(wrong)
    ...
    def test_unrotated_support_polygons_hit_rotated_trellis_keepout(self) -> None:
        ...
        self.assertAlmostEqual(intersection_volume(wrong_components["corner_restraint_nw"], probe), 90.918926, places=5)
        self.assertAlmostEqual(intersection_volume(wrong_components["corner_restraint_sw"], probe), 90.918926, places=5)
    ...
    def test_shallow_lid_has_no_collision_and_measured_clearance(self) -> None:
        shallow = apply_shallow_mating_interface(self.params, cad.build_pristine_shallow_lid(self.params))
        intersection = self.device.intersect(shallow)
        self.assertEqual(sum(s.Volume() for s in cast(list[cq.Shape], intersection.solids().vals())), 0.0)
        self.assertAlmostEqual(cast(cq.Shape, self.device.val()).distance(cast(cq.Shape, shallow.val())), 1.192686, places=5)
