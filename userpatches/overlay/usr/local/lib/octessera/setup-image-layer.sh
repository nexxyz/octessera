...
for setup_file in \
  etc/default/locale \
  usr/local/share/octessera-setup-ui/js/app.js \
  usr/local/share/octessera-setup-ui/index.html \
  usr/local/share/octessera-setup-ui/css/styles.css \
  usr/local/share/octessera-setup-ui/README.md \
  usr/local/share/octessera-setup-ui/img/octessera-mark.svg \
  usr/local/share/octessera-setup-ui/img/octessera-wordmark.svg; do
  install_required "$setup_file" "/$setup_file" 0644
done

install_required usr/local/sbin/octessera-setup /usr/local/sbin/octessera-setup 0755
install_required usr/local/lib/octessera/setup_config.py /usr/local/lib/octessera/setup_config.py 0644
install_required usr/local/lib/octessera/setup_http.py /usr/local/lib/octessera/setup_http.py 0644
install_required etc/octessera/setup-profile /etc/octessera/setup-profile 0644
install_required etc/tmpfiles.d/octessera-setup-request.conf /etc/tmpfiles.d/octessera-setup-request.conf 0644
...
for obsolete in \
  ...
  /etc/tmpfiles.d/octessera-setup-queue.conf \
  /etc/systemd/system/octessera-setup-request.service \
  /etc/systemd/system/multi-user.target.wants/octessera-setup-request.service; do
  rm -f -- "$obsolete"
done
for stale_ui_file in app.js styles.css octessera-mark.svg octessera-wordmark.svg; do
  rm -f "/usr/local/share/octessera-setup-ui/$stale_ui_file"
done
