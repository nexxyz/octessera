...
pub(crate) fn exact_output_device(name: &str) -> Result<Device, DevicesError> {
    ...
    Ok(Device {
        name: name.to_owned(),
        handles: Arc::new(Mutex::new(DeviceHandles::default())),
    })
}
...
fn classify_poll_error(
    revents: alsa::poll::Flags,
    state: alsa::pcm::State,
    avail_errno: Option<libc::c_int>,
) -> PollErrorFlow {
...
fn stream_type_from_revents(revents: alsa::poll::Flags) -> Option<StreamType> {
    if revents.contains(alsa::poll::Flags::OUT) {
        Some(StreamType::Output)
    } else if revents.contains(alsa::poll::Flags::IN) {
...
fn poll_descriptors_and_prepare_buffer(
    ...
    let stream_type = stream_type_from_revents(revents);
    if !revents.contains(alsa::poll::Flags::ERR) && stream_type.is_none() {
        // Nothing to process, poll again
        return Ok(PollDescriptorsFlow::Continue);
    }

    let status = stream.channel.status()?;
    let avail_result = stream.channel.avail();
    if revents.contains(alsa::poll::Flags::ERR) {
    ...
    let avail_frames = match avail_result {
    ...
    let stream_type = match stream_type {
        ...
        None => return Ok(PollDescriptorsFlow::Continue),
    };
...
mod tests {
    ...
    #[test]
    fn pollerr_xrun_uses_recovery_flow_without_repeating_generic_errors() {
        assert_eq!(
            classify_poll_error(alsa::poll::Flags::ERR, alsa::pcm::State::XRun, None),
            PollErrorFlow::XRun
        );
        assert_eq!(
            classify_poll_error(
        ...
        assert_eq!(
            classify_poll_error(alsa::poll::Flags::OUT, alsa::pcm::State::Prepared, None),
            PollErrorFlow::Continue
        );
    }
    ...
    #[test]
    fn pollerr_combined_with_output_still_selects_output() {
        assert_eq!(
            stream_type_from_revents(alsa::poll::Flags::ERR | alsa::poll::Flags::OUT),
            Some(StreamType::Output)
        );
        assert_eq!(
            classify_poll_error(
                alsa::poll::Flags::ERR | alsa::poll::Flags::OUT,
                alsa::pcm::State::Running,
                None,
            ),
            PollErrorFlow::Continue
