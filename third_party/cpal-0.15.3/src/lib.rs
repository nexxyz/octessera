...
//! This vendored CPAL source was modified by Octessera to expose an exact ALSA
//! output PCM construction path without broad device enumeration.
...
pub fn alsa_exact_output_device(name: &str) -> Result<Device, DevicesError> {
    host::alsa::exact_output_device(name).map(Into::into)
}
...
pub fn alsa_exact_output_device(name: &str) -> Result<Device, DevicesError> {
    ...
    Err(BackendSpecificError {
        description: "the exact ALSA output PCM path is unavailable on this target".into(),
    }
    .into())
}
