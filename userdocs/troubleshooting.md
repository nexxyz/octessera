...
| USB audio or MIDI is missing | Use the selected build's host-data port only after its port-role, VBUS/CC, and no-backfeed gates pass; use a data-only or power-isolating cable when the instrument has separate power. | [Safety and power](hardware/safety-and-power.md#usb-host-connections), [pinout and connections](hardware/pinout-and-connections.md#usb-audio-and-midi) |
...
| A sample is missing | Confirm which path you are using: desktop releases need your own samples, Orange stages three defaults, and Raspberry OLED SD samples are user-managed. | [Desktop simulator](desktop-simulator.md#make-a-first-sound), [samples and OLED SD storage](README.md#samples-and-oled-sd-storage), [Orange samples](hardware/orange-pi-first-boot.md#samples-and-output-paths) |
